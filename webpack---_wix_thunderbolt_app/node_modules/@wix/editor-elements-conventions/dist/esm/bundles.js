...
function getComponentName(componentName, originalComponentName) {
  ...
  if (componentName === originalComponentName || !originalComponentName) {
    ...
    return `${componentName.replace(
      new RegExp(originalComponentName, "g"),
      "~"
...
export const parseBundleName = (bundleName) => {
  ...
  const { bundleId, extension } = getModuleIdFromBundle(bundleName);
...
export const getManifestName = ({ namespace, host, isDev }) => {
...
export const parseManifestName = (manifest) => {
  ...
  const result = /(.+)\.manifest(\.min)?\.json$/.exec(
    manifest.replace(BUNDLE_PREFIX, "")
  );
  if (!result) {
    return null;
  }
