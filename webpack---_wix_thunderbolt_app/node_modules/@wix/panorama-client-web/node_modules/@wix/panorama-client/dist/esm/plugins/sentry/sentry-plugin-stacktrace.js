import { extractArtifactDataFromScriptUrl } from "../../utils/artifact-utils";
...
export const extractArtifactsDataFromSentryEvent = (event, hint) => {
  ...
  const frames =
    ...
    (_event$exception = _event$exception.values) == null ||
    (_event$exception = _event$exception[0]) == null ||
    (_event$exception = _event$exception.stacktrace) == null
      ? void 0
      : _event$exception.frames) ||
    splitOriginalStacktraceIntoFrames(
      hint == null ? void 0 : hint.originalException
    );
  for (let i = frames.length - 1; i >= 0; i--) {
    const { filename, raw } = frames[i];
