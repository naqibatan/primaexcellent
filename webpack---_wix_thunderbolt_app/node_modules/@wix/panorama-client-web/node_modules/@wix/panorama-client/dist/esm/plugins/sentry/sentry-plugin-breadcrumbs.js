import { getGlobalHub, getMainHub } from "./sentry-plugin-state";
const getBreadcrumbsFromHub = (hub) => {
  ...
  const values = Object.values(
    (_hub$getScope = hub.getScope()) != null ? _hub$getScope : {}
  );
...
export const getBreadcrumbs = (globalConfig) => {
  const breadcrumbs = [
    ...getBreadcrumbsFromHub(getGlobalHub(globalConfig)),
    ...getBreadcrumbsFromHub(getMainHub(globalConfig)),
  ].map((breadcrumb) => {
    let data;
    try {
      if (breadcrumb.data) {
        data = Object.entries(breadcrumb.data).reduce((res, _ref) => {
          ...
          res[key] = Array.isArray(value)
            ? value.map((val) => (val == null ? void 0 : val.toString()))
            : value == null
            ? void 0
            : value.toString();
          return res;
...
export function enrichBreadcrumbsWithDataHooks(breadcrumb, hint) {
  ...
  const isUiInteractionBreadcrumb =
    category && (category === "ui.click" || category === "ui.input");
  ...
  if (isUiInteractionBreadcrumb && targetElement) {
    ...
    if (nodeWithDataHook) {
      ...
      if (targetElement === nodeWithDataHook) {
        breadcrumb.message =
          breadcrumb.message + ' [data-hook="' + dataHook + '"]';
      } else {
        breadcrumb.message =
          breadcrumb.message + ' parent:[data-hook="' + dataHook + '"]';
      }
    }
  }
  return breadcrumb;
