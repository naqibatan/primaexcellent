import { RoutingAction } from "./sentry-plugin-types";
import { Hook } from "../hooks-manager";
import { defaults } from "../../utils/general-utils";
import { calculateEnvironment } from "../../utils/environment-utils";
import { getArtifactDataFromBaseParams } from "../../utils/artifact-utils";
...
import { enrichBreadcrumbsWithDataHooks } from "./sentry-plugin-breadcrumbs";
import {
  getGlobalHub,
  getHubForDsn,
  getMainHub,
...
import { getRoutingDecision } from "./sentry-plugin-router";
import { extractArtifactsDataFromSentryEvent } from "./sentry-plugin-stacktrace";
const createHub = (options) => {
  const { artifactData, pluginParams } = options;
  const { sentry, sentryTransport, sentryDsn } = pluginParams;
  if (!sentry || !sentryDsn) {
    return {
      hub: null,
    };
  ...
  const hub = new sentry.Hub(
    new sentry.BrowserClient({
      ...
      beforeSend(event, hint) {
        return typeof sentryBeforeSendHook === "function"
          ? sentryBeforeSendHook(event, hint)
          : event;
      },
    })
  );
...
const initHub = (hubResult, options) => {
  const { hub } = hubResult;
  const { artifactData, baseParams, pluginParams, globalConfig } = options;
  const { fullArtifactId, componentId, uuid, msid } = baseParams;
  const { sentry, sentryDsn, sentryMain } = pluginParams;
  const { artifactId } = artifactData;
  if (uuid) {
    hub.setUser({
      id: uuid,
    });
...
const initHooks = (hubResult, options) => {
  const { artifactData, pluginParams, baseClient, globalConfig, hooksManager } =
    options;
  const { sentryBeforeSend } = hubResult;
  const reportErrorToPanoramaSafely =
    createSafePanoramaErrorReporter(baseClient);

  // Subscribe to Sentry error reports
  sentryBeforeSend((event, hint) => {
    const error = normalizeError(
      event,
      hint == null ? void 0 : hint.originalException
    );
    if (
      isUnprocessedUnhandledError(event) &&
      !hooksManager.invoke(Hook.beforeUnhandledError, error)
    ) {
      return null;
    }
    const stackTraceArtifactsData = extractArtifactsDataFromSentryEvent(
      event,
      hint
    );
    let panoramaData = extractPanoramaDataFromSentryEvent(event);
    const { routingAction: prevRoutingAction } = panoramaData;
    ...
    if (prevRoutingAction === RoutingAction.REROUTE) {
      ...
      const { routingAction, routingActionReason, targetHub } =
        getRoutingDecision({
          artifactData,
    ...
    if (reportToPanorama) {
      panoramaData = enrichPanoramaDataWithSentryEventData(
        panoramaData,
        event,
        {
          stackTraceArtifactsData,
    ...
    if (reportToSentry) {
      if (pluginParams.sentryGetReleaseFromStacktrace) {
        event = setReleaseFromStackTraceArtifactsData(
          event,
          artifactData,
          stackTraceArtifactsData
        );
      ...
      event = enrichSentryEventWithPanoramaData(
        event,
        panoramaData,
        baseClient
      );
  ...
  hooksManager.tap(
    ...
    (error, data, errorMonitorOptions) => {
      ...
      if (
        errorMonitorOptions != null &&
        (_errorMonitorOptions$ = errorMonitorOptions.pluginParams) != null &&
        _errorMonitorOptions$.sentryLookupDsn
      ) {
        var _errorMonitorOptions$2;
        const hub = getHubForDsn(
          errorMonitorOptions == null ||
            (_errorMonitorOptions$2 = errorMonitorOptions.pluginParams) == null
            ? void 0
            : _errorMonitorOptions$2.sentryLookupDsn,
          globalConfig
        );
        if (hub) {
          sentryHub = hub;
  ...
  hooksManager.tap(Hook.beforeAddBreadcrumb, (breadcrumb) => {
    var _getGlobalHub;
    const hub =
      (_getGlobalHub = getGlobalHub(globalConfig)) != null
        ? _getGlobalHub
        : getMainHub(globalConfig);
...
export const sentryPlugin = function (pluginFactoryParams) {
  ...
  return (_ref) => {
    let { baseParams, pluginParams, baseClient, hooksManager, globalConfig } =
      _ref;
