...
import { RoutingAction, RoutingActionReason } from "./sentry-plugin-types";
import {
  getFirstMatchingHub,
  isScriptTypeBlacklisted,
} from "./sentry-plugin-utils";
import { isArtifactMismatch, isSameArtifact } from "../../utils/artifact-utils";
export const getRoutingDecision = (options) => {
  const { artifactData, stackTraceArtifactsData, panoramaData, globalConfig } =
    options;
  const { exceptionType } = panoramaData;
  ...
  if (!result) {
    return {
      routingAction: RoutingAction.DROP,
      routingActionReason:
        RoutingActionReason.NO_REGISTERED_ARTIFACTS_IN_STACKTRACE,
    };
  }
  const { hub, artifactData: matchedArtifactData } = result;
  ...
  const routingActionReason = isSameArtifact(
    matchedArtifactData,
    stackTraceArtifactsData[0]
  )
    ? RoutingActionReason.EXACT_REROUTE_MATCH
    : RoutingActionReason.FALLBACK_REROUTE_MATCH;
  return {
