...
import { getHubForArtifact } from "./sentry-plugin-state";
import { getFirstMatchingArtifact } from "../../utils/artifact-utils";

/**
 * Safe Panorama logger to be used within Sentry's "beforeSend" hook so we don't trigger report loops if the call fails
 */
export const createSafePanoramaLogger =
  (baseClient) => (logLevel, message, data) => {
    try {
 ...
 */
export const createSafePanoramaErrorReporter =
  (baseClient) => (error, panoramaData) => {
    ...
    try {
      ...
      createSafePanoramaLogger(baseClient)(
        LogLevel.Error,
...
export const isUnprocessedUnhandledError = (event) => {
  var _event$contexts;
  return !(
    (_event$contexts = event.contexts) != null &&
    _event$contexts[PANORAMA_DATA_KEY]
  );
};
export const isScriptTypeBlacklisted = (stackTraceArtifactsData) =>
  stackTraceArtifactsData.some((_ref) => {
    let { scriptType } = _ref;
    return BlacklistedStacktraceScriptTypes.includes(scriptType);
  });
export const buildReleaseString = (artifactData) => {
  const { artifactId, artifactVersion } = artifactData;
  return artifactId + "@" + artifactVersion;
};
export const setReleaseFromStackTraceArtifactsData = (
  event,
  currentArtifactData,
  stackTraceArtifactsData
) => {
  const artifact = getFirstMatchingArtifact(
    currentArtifactData,
    stackTraceArtifactsData
  );
  if (!artifact) {
...
export const getFirstMatchingHub = (stackTraceArtifactsData, globalConfig) => {
  for (const stackTraceArtifactData of stackTraceArtifactsData) {
    const hub = getHubForArtifact(
      stackTraceArtifactData.artifactId,
      globalConfig
    );
    if (hub) {
      return {
