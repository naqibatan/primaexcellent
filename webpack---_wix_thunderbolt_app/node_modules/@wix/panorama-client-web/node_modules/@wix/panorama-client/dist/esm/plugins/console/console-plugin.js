...
import { fullArtifactIdToArtifactId } from "../../utils/artifact-utils";
const getWixStaticsVersionsCookieValue = () => {
  ...
  const cookie = document.cookie
    .split(";")
    .map((c) => c.trim())
    .find((c) => c.startsWith("wixStaticsVersions="));
...
const hasArtifactIdOverrideQueryParam = (artifactId) => {
  ...
  return Array.from(queryParams.keys()).some((key) =>
    key.startsWith(queryParamKeyPrefix)
  );
};
...
const checkIsLocalMode = (fullArtifactId) => {
  ...
  return (
    (wixStaticCookieValue && wixStaticCookieValue.includes(artifactId)) ||
    hasArtifactIdOverrideQueryParam(artifactId)
  );
};
export const consolePlugin = () => (_ref) => {
  let {
    hooksManager,
    baseParams: { fullArtifactId, componentId },
  } = _ref;
