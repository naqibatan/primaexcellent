...
var WixDataQueryReferencedResult = /** @class */ (function () {
  ...
  WixDataQueryReferencedResult.prototype._rejectNoPrevPage = function () {
    return Promise.reject(
      validationError(
        messages.queryValidations.noPrevPage(this._collectionName)
      )
    );
  };
  WixDataQueryReferencedResult.prototype._rejectNoNextPage = function () {
    return Promise.reject(
      validationError(
        messages.queryValidations.noNextPage(this._collectionName)
      )
    );
  };
  WixDataQueryReferencedResult.prototype._withChangedSkip = function (isNext) {
    ...
    return referenceResultFactory(
      ...
      __assign(__assign({}, this._options), {
        skip: newSkip,
        limit: this._limit,
      })
    );
  };
  WixDataQueryReferencedResult.prototype._getNewCursorQuery = function (
    cursor
  ) {
    return referenceResultFactory(
      {},
      this._provider,
      this._collectionName,
      this._holdingItemId, // Needed for cloud-data as for validations at the beginning of the pipeline. TODO: fix validations
    ...
    return this._provider
      ...
      .then(function (result) {
        return referenceResultFactory(
          result,
          _this._provider,
          _this._collectionName,
          _this._holdingItemId,
          _this._referenceAttribute,
          _this._options
        );
      });
  };
  WixDataQueryReferencedResult.prototype._withTotalCount = function (
    oldTotalCount
  ) {
...
export { WixDataQueryReferencedResult };
export function referenceResultFactory(
  result,
  provider,
  collectionName,
  itemId,
