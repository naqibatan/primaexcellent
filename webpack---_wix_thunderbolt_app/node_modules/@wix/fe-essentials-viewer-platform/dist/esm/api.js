...
export class ViewerPlatformEssentials {
  constructor(env) {
    ...
    this.createPanoramaClient = (options) => {
      const { fullArtifactId, artifactVersion, componentId, sentryDsn } =
        options;
      const { basePanoramaClientFactory, experiments } = this;
  ...
  createBasePanoramaClientFactory() {
    ...
    const { isSSR, metaSiteId, fetch, Sentry, sentryTransport } = env;
    const factory = panoramaClientFactory({
      ...
      pluginParams: {
        sentry:
          experiments.get("specs.thunderbolt.PanoramaErrorMonitor") === true &&
          !isSSR
            ? Sentry
            : undefined,
        sentryTransport,
  ...
  createAppEssentials(env) {
    return new _AppEssentials({
      env: Object.assign(Object.assign({}, env), { platformEnv: this.env }),
      essentials: this,
    });
  }
  /**
  ...
  createControllerEssentials(env, appEssentials) {
    return new _ControllerEssentials({
      ...
      env: Object.assign(Object.assign({}, env), { appEnv: appEssentials.env }),
      essentials: this,
      appEssentials,
    });
  }
...
export class _AppEssentials {
  constructor({ env, essentials }) {
    ...
    this.createErrorMonitor = (options) => {
      ...
      if (
        this.essentials.experiments.get(
          "specs.thunderbolt.PanoramaErrorMonitor"
        ) === true &&
        !isSSR
      ) {
        const { fullArtifactId, componentId, dsn, version } = options;
        const panoramaClient = this.createPanoramaClient({
          ...
          data: {
            tags: Object.assign(
              { appDefinitionId: appDefId, instanceId },
              options.tags
            ),
          },
        });
        ...
        return {
          captureException(e, opts) {
            errorMonitor.reportError(e, {
              ...
              context:
                opts === null || opts === void 0 ? void 0 : opts.contexts,
            });
            return "";
          },
        ...
        return createRavenClient(Object.assign({ Raven }, options));
      }
    };
    /**
    ...
    this.experiments = new ReadOnlyExperiments({
      experiments:
        (_a =
          appsConductedExperiments === null ||
          appsConductedExperiments === void 0
            ? void 0
            : appsConductedExperiments[appDefId]) !== null && _a !== void 0
          ? _a
          : {},
    });
    this.httpClient = createHttpClient({
      ...
      getAppToken: () =>
        getAppToken(
          shouldElevateBlocksAppPermissions ? METASITE_APP_DEF_ID : appDefId
        ),
    });
    this.createCurrencyFormatterWithDefaults =
      createCurrencyFormatterWithDefaults;
...
export class _ControllerEssentials {
  constructor({ env, appEssentials }) {
     ...
     * A bi Logger and Factory for more loggers
     */
    this.biLoggerFactory = (options) =>
      this.appEssentials.biLoggerFactory(options);
    /**
     * An ErrorMonitor factory (@wix/error-monitor)
     */
    this.createErrorMonitor = (options) => {
     ...
     * An Experiments object factory
     */
    this.createExperiments = (options) =>
      this.appEssentials.createExperiments(options);
    this.env = env;
