...
import { validateSiteAssetsRequest } from "./clientValidation";
import { keys } from "../helper/objectHelper";
export var UrlBuilder = function (siteModels, isStagingRequest) {
  ...
  var experimentsParam = function (experiments) {
    ...
    if (experimentKeys.length !== 0) {
      return { experiments: experimentKeys.sort().join(",") };
    } else return {};
  };
  var buildSiteModelQueryParams = function (siteModels, moduleName) {
    ...
    var csmCacheKeyParam = siteModels.metaSiteModel.csmCacheKey
      ...
      : {};
    var siteRevisionConfigKeyParam = siteModels.sitePagesModel
      .siteRevisionConfig
      ? {
          commonConfig: JSON.stringify(
            siteModels.sitePagesModel.siteRevisionConfig
          ),
        }
      : {};
    var moduleCacheVersionParam = cacheVersions
      ? cacheVersions[moduleName]
    ...
    return __assign(
      __assign(
        __assign(
          __assign(__assign({}, requiredParams), experimentsParam(experiments)),
          csmCacheKeyParam
        ),
        siteRevisionConfigKeyParam
      ),
      moduleCacheVersionParam
    );
  };
  var contentTypeParam = function (contentType) {
    if (contentType) return { contentType: contentType };
    else return {};
  };
  var siteAssetsCacheKillerParam = function (disableCache) {
    if (disableCache) return { sack: "".concat(getRandomInt()) };
    else return {};
  };
  var isStagingParam = function (isStagingRequest) {
    if (isStagingRequest) return { isStaging: "1" };
    else return {};
  };
  var versionParam = function (fetchType, version) {
    if (fetchType === "module") return { moduleVersion: version };
    else if (fetchType === "file") return { fileId: version };
    else return {};
  };
  var buildRequestQueryParams = function (request) {
    ...
    return __assign(
      __assign(
        __assign(
          __assign(
            __assign(__assign(__assign({}, params), { module: name }), {
              pageId: request.pageJsonFileName,
            }),
            contentTypeParam(request.contentType)
          ),
          versionParam(fetchType, version)
        ),
        siteAssetsCacheKillerParam(request.disableSiteAssetsCache)
      ),
      isStagingParam(isStagingRequest)
    );
  };
  var siteAssetsParams = function (request) {
    validateSiteAssetsRequest(request);
    var siteModelQueryParams = buildSiteModelQueryParams(
      siteModels,
      request.module.name
    );
  ...
  var siteAssetsUrl = function (request, siteAssetsBaseDomain) {
    ...
    return ""
      .concat(request.urlOverride || siteAssetsBaseDomain)
      .concat(endpoint, "?")
      .concat(paramsQueryStr);
  };
  return {
