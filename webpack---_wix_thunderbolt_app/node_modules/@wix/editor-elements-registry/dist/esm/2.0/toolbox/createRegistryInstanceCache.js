import { LOCAL_DEV_HOSTS } from "../utils";
export function createRegistryInstanceCache({ cache = new Map() } = {}) {
  const pending = new Map();
  return {
    async getRegistryAPI({ libraries, factory, shouldCache = true }) {
      ...
      try {
        ...
        const withLocalURLs = normalized.some((library) => {
          return LOCAL_DEV_HOSTS.some((host) => library.url.startsWith(host));
        });
