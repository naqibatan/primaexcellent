import { getBundleId } from "@wix/editor-elements-conventions";
import { isJS, isCSS, getExtension } from "../utils";
...
export function createManifestAPI(manifest) {
  ...
  function extractModelSources(urls, { componentName, part, batch } = {}) {
    const model = {};
  ...
  return {
    ...
    getParts() {
      ...
      Object.entries(manifest.batches ?? {}).forEach(([batchId, batch]) => {
        ...
        const url = Object.keys(batch.url_v2 || []).length
          ? batch.url_v2
          : batch.url;
    ...
    getComponents() {
      ...
      Object.keys(manifest.components ?? {}).forEach((componentName) => {
        ...
        components[componentName] = {
          ...
          name: componentName,
          src: extractModelSources(urls, { componentName }),
        };
      });
      Object.entries(manifest.batches ?? {}).forEach(([batchId, batch]) => {
        const isBundlesConcatenated = batch.url_v2 && batch.url_v2.length;
