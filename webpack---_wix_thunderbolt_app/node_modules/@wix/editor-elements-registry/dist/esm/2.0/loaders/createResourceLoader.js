import { isBrowser, isServer, isWorker, LOCAL_DEV_HOSTS } from "../utils";
import {
  evaluateBundle,
  getModel,
  getModelFromContext,
...
export function createResourceLoader({ options, globals, fetcher = _fetcher }) {
  const contexts = {};
  const loadResource = ({ id, url }) =>
    withRetry({
      times: RETRY_TIMES,
      task: async () => {
    ...
    createBundleLoader({ environment }) {
      ...
      const fetchEnvironment = async () => {
        ...
        pendingEnvironmentPromise = Promise.all([
          ...
          isBrowser()
            ? load({ id: HOT_BUNDLE_ID, url: environment.hot })
            : null,
        ]).then(() =>
          /**
