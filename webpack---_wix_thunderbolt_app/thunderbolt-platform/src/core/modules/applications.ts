...
const Applications = (
	...
	const runApplication = async (appDefinitionId: string) => {
		...
		const batchQueueBi = experiments['specs.thunderbolt.oneQueueForLoggers'] ? createBatchQueue() : undefined
		const batchQueueFedops = experiments['specs.thunderbolt.oneQueueForLoggers'] ? createBatchQueue() : undefined
