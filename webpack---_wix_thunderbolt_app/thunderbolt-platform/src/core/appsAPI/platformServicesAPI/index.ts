...
const createLoggerFactoriesForApp: CreateLoggerFactoriesForApp = ({
	...
	const biBaseFactory = () => {
		...
		if (experiments['specs.thunderbolt.oneQueueForLoggers']) {
			if (batchQueueBi) {
				base.setGlobalBatchQueue(batchQueueBi)
			}
		} else {
			base.setGlobalBatchQueue(createBatchQueue())
		}
	...
	if (experiments['specs.thunderbolt.oneQueueForLoggers']) {
		...
		fedopsBiLoggerFactory.setGlobalBatchQueue(createBatchQueue())
	}
