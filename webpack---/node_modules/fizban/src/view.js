 ...
 * @return {ScrollScene}
 */
function transformSceneRangesToOffsets(
  scene,
  rect,
  viewportSize,
  isHorizontal,
  ...
  if (typeof duration === "string") {
    ...
    if (startRange || start?.name) {
      ...
      const startAdd = transformAbsoluteOffsetToNumber(
        startRange.add,
        absoluteOffsetContext
      );
      startOffset =
        transformRangeToPosition(startRange, viewportSize, rect) + startAdd;
    }

    if (endRange || end?.name) {
      endRange = endRange || end;
      const endAdd = transformAbsoluteOffsetToNumber(
        endRange.add,
        absoluteOffsetContext
      );
      endOffset =
        transformRangeToPosition(endRange, viewportSize, rect) + endAdd;
 ...
 * @return {ScrollScene}
 */
export function getTransformedScene(
  scene,
  root,
  viewportSize,
  isHorizontal,
  ...
  const elementStickiness = isElementSticky
    ? getStickyData(elementStyle, isHorizontal)
    : undefined;
  ...
  while (parent) {
    ...
    const sticky = isSticky
      ? getStickyData(nodeStyle, isHorizontal)
      : undefined;
    ...
    if (sticky) {
      if ("end" in sticky) {
        ...
        if (parent) {
           ...
           */
          const stuckStart =
            accumulatedOffset + offsetFromViewEnd - node.offset;
          ...
          const isInsideDuration =
            !isBeforeStart && stuckStart <= transformedScene.end;
      ...
      if ("start" in sticky) {
        ...
        if (parent) {
          if (isBeforeStart || isInsideDuration) {
            const parentSize =
              (isHorizontal ? parent.offsetWidth : parent.offsetHeight) || 0;
