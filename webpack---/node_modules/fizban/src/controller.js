...
export function getController(config) {
  ...
  let viewportObserver,
    rangesResizeObserver,
    viewportResizeHandler,
    scrollportResizeObserver;
  ...
  _config.scenes = config.scenes.map((scene, index) => {
    ...
    if (
      scene.viewSource &&
      (typeof scene.duration === "string" || scene.start?.name)
    ) {
      scene = getTransformedScene(
        scene,
        root,
        viewportSize,
        horizontal,
    ...
    if (window.ResizeObserver) {
      ...
      rangesResizeObserver = new window.ResizeObserver(function (entries) {
        entries.forEach((entry) => {
          ...
          _config.scenes[scene.index] = getTransformedScene(
            scene,
            root,
            viewportSize,
            horizontal,
          ...
          rangesToObserve.splice(
            rangesToObserve.indexOf(scene),
            1,
            _config.scenes[scene.index]
          );
        });
      });
    ...
    if (_config.observeViewportResize) {
      viewportResizeHandler = debounce(function () {
        ...
        const newRanges = rangesToObserve.map((scene) => {
          const newScene = getTransformedScene(
            scene,
            root,
            viewportSize,
            horizontal,
      ...
      if (root === window) {
        ...
        scrollportResizeObserver = new window.ResizeObserver(
          viewportResizeHandler
        );
