import { gradientConstants } from "./constants.js";
const { GRADIENT_LINEAR, GRADIENT_CONIC, GRADIENT_CIRCLE } =
  gradientConstants.TYPES;
const { RADIAL_GRADIENT, CONIC_GRADIENT, LINEAR_GRADIENT } =
  gradientConstants.CSS_FUNC;
const { REPEATING, DEG, AT_CENTER, CIRCLE, ELLIPSE, FROM, PERCENTAGE } =
  gradientConstants;
/**
 *
 ...
 */
function generateLinear({ angle = 0 }) {
 ...
 */
function generateRadial({ size, position, type }) {
 ...
 */
function generateConic({ angle = 0, position, type }) {
  const gradientPosition = generateGradientPosition(position, type);
  const gradientAngle = generateGradientAngle(angle);
 ...
 */
function generateUnitSize({ type, value }) {
  if (type === PERCENTAGE) {
    return `${value}%`;
  }
  return `${value}${type}`;
