...
function isWixCodeConsoleMessage(msg) {
  return (
    msg.intent === WIX_CODE_INTENT && msg.type === WIX_CODE_CONSOLE_MESSAGE_TYPE
  );
}
...
function logWixCodeConsoleError(error) {
  if (isPreview()) {
    var wixCodeConsoleMessage = convertToWixCodeConsoleMessage(
      logLevels.ERROR,
      error.name,
      error.message,
      error.stack
