import { isBrowser, LOCAL_DEV_HOSTS } from "../utils";
import { loadAssets } from "./loadAssets";
import { createPromiseGuard, getScriptTag, withRetry } from "./utils";
const RETRY_TIMES = 3;
/**
...
const _fetchEnvironment = async (environment, loadResource) => {
  ...
  return Promise.all([
    load({ url: environment.runtime }),
    isBrowser() ? load({ id: HOT_BUNDLE_ID, url: environment.hot }) : null,
  ]);
};
export function createEsmResourceLoader({ fetcher = _fetcher }) {
...
`;
    const blob = new Blob([resCode], { type: "text/javascript" });
    const resourceUrl = URL.createObjectURL(blob);
    const exp = await import(resourceUrl);
    URL.revokeObjectURL(resourceUrl);
    const res = await exp.default;
    return res;
  };
  const loadScriptWithRetry = (url) =>
    withRetry({
      times: RETRY_TIMES,
  ...
  return {
    ...
    async fetchLibraryManifest({ url }) {
      const response = await fetcher(url);
      return JSON.parse(response);
    },
    createBundleLoader({ environment }) {
