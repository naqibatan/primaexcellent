import { isCSS, isJS, yieldToMain } from "../utils";
import { createException, RegistryErrorCode } from "../createException";
export function getComponentAssets({ resource, parts = {} }) {
  const assets = [];
...
export function createComponentLoader({
  ...
  const loader = async (part) => {
    ...
    if (resource.src.deps) {
      await Promise.all([
        ...resource.src.deps.filter(isJS).map((dep) =>
          loadBundle({
            url: dep,
          })
        ),
    ...
    if (bundles.some((bundle) => !bundle || !bundle.components)) {
      throw createException(
        RegistryErrorCode.NoComponentsAtComponentBundle,
        "",
        {
    ...
    const model = Object.assign(
      {},
      ...bundles.map((bundle) =>
        bundle ? bundle.components[resource.name] : {}
      )
    );
  ...
  loader.assets = getComponentAssets({ resource, parts });
  return loader;
}
export async function createLazyComponentLoaders({ manifest, loadBundle }) {
...
export async function createEagerComponentLoaders({ manifest, loadBundle }) {
  const components = manifest.getComponents();
  const parts = manifest.getParts();
