...
function parseTagName(
  ...
  var indexOfTagNameEnd =
    indexOfFirstSpaceAfterOpenBracket || currentTokenIndex;
  ...
  if (lastBracketToken === OPEN_TAG_START) {
    ...
    if (selfClosingTagsSet.has(tagName)) {
      callbacks.onSelfClosingTag &&
        callbacks.onSelfClosingTag({ tagName: tagName, props: props });
    } else {
      callbacks.onOpenTag &&
        callbacks.onOpenTag({ tagName: tagName, props: props });
    }
  } else {
    callbacks.onClosingTag &&
      callbacks.onClosingTag({ tagName: tagName, props: "" });
  }
}
  ...
  while (i <= htmlLength) {
    ...
    switch (currentToken) {
      ...
      case SPACE:
        if (
          !indexOfFirstSpaceAfterOpenBracket &&
          (lastBracketToken === OPEN_TAG_START ||
            lastBracketToken === CLOSING_TAG_START)
        ) {
