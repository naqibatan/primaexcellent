...
const parseVideoPosters = (fileOutput) =>
  fileOutput.image.map((image) => url2uri(image.url));
...
const parseVideoQualities = (fileOutput) => {
  ...
  const storyboard =
    (_fileOutput$storyboar = fileOutput.storyboard) == null
      ? void 0
      : _fileOutput$storyboar.find((_ref3) => {
          let { format } = _ref3;
          return format === "mp4";
        });
  const qualities = mp4Videos.map((_ref4) => {
    let { width, height, quality, url } = _ref4;
    return {
      width,
      height,
      quality,
...
const parseMediaFeatures = (fileInfo) => {
  ...
  const mediaFeatures = [];
  if (
    (_fileInfo$tags = fileInfo.tags) != null &&
    _fileInfo$tags.includes("_mp4_alpha")
  ) {
...
export const mediaManagerDefaultPaths = {
  Color:
    "public/6ae967c404004038b5e84dd0a6ab2b65/73d89c137a524437880b113939ca24c3",
  // fallback to image
  Image:
    "public/6ae967c404004038b5e84dd0a6ab2b65/73d89c137a524437880b113939ca24c3",
  WixVideo:
...
export const parseVideoFileInfo = (fileInfo, info) => {
  ...
  const videoId = getIdFromUrl(
    getObjectValueByKey(fileInfo, "file_name") ||
      getObjectValueByKey(fileInfo, "file_url")
  );
  ...
  return {
    ...
    fps: ((_fileOutput$video$0$f =
      (_fileOutput$video$ = fileOutput.video[0]) == null
        ? void 0
        : _fileOutput$video$.fps) != null
      ? _fileOutput$video$0$f
      : ""
    ).toString(),
...
export function getImageBackgroundObject(fileInfo, info) {
  ...
  return {
    ...
    hasAnimation: !!(
      (_fileInfo$file_input = fileInfo.file_input) != null &&
      _fileInfo$file_input.animated
    ),
  };
}
export function getImageBackgroundObjectFromMediaStudioFile(
  mediaStudioFileInfo
) {
  return {
    type: "Image",
...
export function isSmallerFromContainer(
  width,
  height,
  containerWidth,
  containerHeight
...
export function getMediaType(background) {
  ...
  return (_background$mediaRef$ =
    background == null || (_background$mediaRef = background.mediaRef) == null
      ? void 0
      : _background$mediaRef.type) != null
    ? _background$mediaRef$
    : MEDIA_TYPES.COLOR;
}
...
export function getStoryboardDisplayValue(videoData, currentValue) {
  ...
  const storyboardItem = videoData.qualities.find(
    (quality) => quality === "storyboard"
  );
...
export function getOverlayStyle(data, colorResolver) {
  ...
  return {
    backgroundImage: data.imageOverlay
      ? "url(" + getMediaURl(data.imageOverlay.uri) + ")"
      : null,
...
export function getColorOrGradient(data) {
  ...
  const fill =
    ((_data$colorLayers = data.colorLayers) == null
      ? void 0
      : _data$colorLayers[0].fill) ||
    ((_data$color = data.color) == null
      ? void 0
      : _data$color.replace(/[{}]/g, ""));
  return typeof fill !== "string" &&
    (fill == null ? void 0 : fill.type) === "SolidColor"
    ? fill.color
    : fill;
}
...
export function getFirstColorStopColor(value, colorResolver) {
  ...
  if (typeof value === "string") {
    ...
    color =
      (_value$gradients = value.gradients) == null
        ? void 0
        : _value$gradients[0].colorStops[0].color;
  } else {
    var _value$colorStops;
    color =
      value == null ||
      (_value$colorStops = value.colorStops) == null ||
      (_value$colorStops = _value$colorStops[0]) == null
        ? void 0
        : _value$colorStops.color;
  }
...
export function getColorOpacityValue(data) {
  ...
  return (_ref5 =
    (_data$colorLayers$0$o =
      (_data$colorLayers2 = data.colorLayers) == null
        ? void 0
        : _data$colorLayers2[0].opacity) != null
      ? _data$colorLayers$0$o
      : data.colorOpacity) != null
    ? _ref5
    : 1;
}
export function getNewColorLayers(data, value, opacity) {
  ...
  let colorLayers;
  opacity =
    (_opacity = opacity) != null ? _opacity : getColorOpacityValue(data);
  if (typeof value === "string") {
    var _value;
    colorLayers = [
      generateColorLayer(
...
export const removeIdsAndMetaDataFromBg = (backgroundData) => {
  ...
  if (backgroundData.mediaRef) {
    ...
    if (backgroundData.mediaRef.posterImageRef) {
      const posterImageRef = omitIdAndMetaData(
        backgroundData.mediaRef.posterImageRef
      );
...
export function getMediaURl(mediaUri, staticMediaUrlOverride) {
  return (
    "" +
    joinURL(
      staticMediaUrlOverride != null
        ? staticMediaUrlOverride
        : getStaticMediaUrl(),
      mediaUri
    )
  );
