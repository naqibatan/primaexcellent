...
function isSEOBot(options) {
  var _a;
  return (_a =
    options === null || options === void 0 ? void 0 : options.isSEOBot) !==
    null && _a !== void 0
    ? _a
    : false;
}
...
function getAlignedRect(sRect, dRect, sFP, alignment) {
  ...
  const x = Math.max(
    0,
    Math.min(sRect.width - dRect.width, fp.x * sRect.width - dRect.width / 2)
  );
  const y = Math.max(
    0,
    Math.min(
      sRect.height - dRect.height,
      fp.y * sRect.height - dRect.height / 2
    )
  );
  // rect
  return {
