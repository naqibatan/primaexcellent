...
function isObject(obj) {
  ...
  return (
    type === "function" || (type === "object" && !Array.isArray(obj) && !!obj)
  );
}
const validateTypes = {
  ...
  elements(key, validator, param) {
    ...
    return toArray(param).every((element) =>
      validateTypes.element(key, validator, element)
    );
  },
};
/**
...
function validateSchema(schema, params, logger) {
  const errors = Object.entries(schema)
    .map(([key, validator]) => {
      ...
      const isValid =
        typeof value !== "undefined" && validateType
