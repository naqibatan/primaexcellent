import { deg2rad, mapRange } from "@wix/wow-utils";
/**
 * Get clipPath values by direction
 */
function getClipParams(
  compRect,
  contentRect,
  direction,
  {
...
function getClipPolygonParams(
  ...
  const left =
    ...
    ((1 - scaleX) / 2) * 100;
  const right =
    (contentRect.width / compRect.width) * 100 + left - (1 - scaleX) * 100; // eslint-disable-line no-extra-parens
  const bottom =
    (contentRect.height / compRect.height) * 100 + top - (1 - scaleY) * 100; // eslint-disable-line no-extra-parens
  ...
  const clipParams = {
    ...
    center: `${centerX - minimum / 2}% ${centerY - minimum / 2}%, ${
      centerX + minimum / 2
    }% ${centerY - minimum / 2}%, ${centerX + minimum / 2}% ${
      centerY + minimum / 2
    }%, ${centerX - minimum / 2}% ${centerY + minimum / 2}%`,
    bottom: `${left}% ${bottom - minimum}%, ${right}% ${
      bottom - minimum
    }%, ${right}% ${bottom}%, ${left}% ${bottom}%`,
    left: `${left}% ${top}%, ${left + minimum}% ${top}%, ${
      left + minimum
    }% ${bottom}%, ${left}% ${bottom}%`,
    vertical: `${left}% ${top + minimum / 2}%, ${right}% ${
      top + minimum / 2
    }%, ${right}% ${bottom - minimum / 2}%, ${left}% ${bottom - minimum / 2}%`,
    horizontal: `${left + minimum / 2}% ${top}%, ${
      right - minimum / 2
    }% ${top}%, ${right - minimum / 2}% ${bottom}%, ${
      left + minimum / 2
    }% ${bottom}%`,
  };
  return {
...
function getClipInsetParams(
  ...
  const top =
    contentRect.top - compRect.top + (compRect.height * (1 - scaleY)) / 2;
  const left =
    contentRect.left - compRect.left + (compRect.width * (1 - scaleX)) / 2;
  const right =
    compRect.width -
    contentRect.width -
    left +
  ...
  const clipParams = {
    ...
    center: `${(top + height / 2) * min}px ${(right + width / 2) * min}px ${
      (bottom + height / 2) * min
    }px ${(left + width / 2) * min}px`,
    bottom: `${(height + top) * min}px ${right}px ${bottom}px ${left}px`,
    left: `${top}px ${(width + right) * min}px ${bottom}px ${left}px`,
    vertical: `${(top + height / 2) * min}px ${right}px ${
      (bottom + height / 2) * min
    }px ${left}px`,
    horizontal: `${top}px ${(right + width / 2) * min}px ${bottom}px ${
      (left + width / 2) * min
    }px`,
  };
  return {
...
function getAdjustedDirection(paramsMap, direction, angleInDeg) {
  ...
  const newIndex =
    (index + (directions.length - 1) * shiftBy) % directions.length; // eslint-disable-line no-mixed-operators
...
function getElementTransformedPosition(origin, compRect, angleInRad) {
  ...
  const toX =
    centerX - centerX * Math.cos(angleInRad) + centerY * Math.sin(angleInRad);
  const toY =
    centerY - centerX * Math.sin(angleInRad) - centerY * Math.cos(angleInRad);
  const fromX =
    originX - originX * Math.cos(angleInRad) + originY * Math.sin(angleInRad);
  const fromY =
    originY - originX * Math.sin(angleInRad) - originY * Math.cos(angleInRad);
  const x = toX - fromX;
  const y = toY - fromY;
  return { x, y };
}
...
function getTransformTweenParams(
  contentRect,
  originDirection,
  angleInRad,
  scale = 1
...
function getRotatedBoundingRectScale(width, height, angleInDeg) {
  const angleInRad = deg2rad(angleInDeg);
  const newHeight =
    width * Math.abs(Math.sin(angleInRad)) +
    height * Math.abs(Math.cos(angleInRad));
  const newWidth =
    width * Math.abs(Math.cos(angleInRad)) +
...
function getMaxRotateBounds(width, height, angleInDeg) {
  ...
  const boundingHeight =
    width * Math.abs(Math.sin(angleInRad)) +
    height * Math.abs(Math.cos(angleInRad));
  const minimalWidth = Math.ceil(
    angleInRad < travelAngle ? boundingWidth : radius * 2
  );
  const minimalHeight = Math.ceil(
    angleInRad < deg2rad(90) - travelAngle ? boundingHeight : radius * 2
  );
  return {
    width: minimalWidth,
...
const TRAVEL_TYPES = {
  ...
  legacy_in: (travel, { maxTravelHeight, travelLastFold, travelFirstFold }) =>
  ...
  in_last_fold: (travel, { maxTravelHeight, travelLastFold }) =>
  ...
  out_first_fold: (travel, { maxTravelHeight, extraOutDistance }) =>
 ...
 */
function getTravelMap(
  componentHeight,
  componentTop,
  siteHeight,
  viewPortHeight,
  getRelativeTravel = TRAVEL_TYPES.normal
) {
  ...
  const travels = {
    ...
    CENTER_TO_CENTER: getRelativeTravel(
      viewPortHeight * 0.5 + componentHeight * 0.5,
      travelParams
    ),
    CENTER_TO_TOP: getRelativeTravel(
      viewPortHeight + componentHeight * 0.5,
      travelParams
    ),
    BOTTOM_TO_BOTTOM: getRelativeTravel(componentHeight, travelParams),
    BOTTOM_TO_CENTER: getRelativeTravel(
      viewPortHeight * 0.5 + componentHeight,
      travelParams
    ),
    BOTTOM_TO_TOP: getRelativeTravel(
      viewPortHeight + componentHeight,
      travelParams
    ), // eslint-disable-line key-spacing
  };
  const withOffset = (travel, percentageOfVh) =>
    Math.min(
      1,
      Math.max(0, travel + (travels.TOP_TO_TOP * percentageOfVh) / 100)
    );
  const isInFirstFold = componentTop < viewPortHeight;
  const isInLastFold =
    siteHeight - (componentTop + componentHeight) < viewPortHeight;
