...
import { getDistance } from "../../../utils/animationsUtils";
const name = "DVD";
const properties = {
  schema: {},
};
...
function getFramesDuration(points, duration, containerWidth, containerHeight) {
  const containerDiagonal = Math.sqrt(
    Math.pow(containerHeight, 2) + Math.pow(containerWidth, 2)
  );
...
function getTravelPoints(
  ...
  const distances = {
    ...
    [`${RIGHT}${DOWN}`]: (x, y) =>
      Math.min(containerHeight - (y + height), containerWidth - (x + width)),
  };
  while (
    getDistance(endPoint, points[points.length - 1]) >= precision ||
...
function animate(kit, elements, duration, delay, _a = {}) {
  ...
  elements.forEach((element) => {
    ...
    const durations = getFramesDuration(
      points,
      duration,
      containerWidth,
      containerHeight
    ...
    points.forEach(({ x, y }, index) => {
      ...
      const { spinDeg, scale } = powerMap[power];
      sequence.add([
