import { __rest } from "tslib";
const name = "BaseBgPositionX";
const properties = {
  hideOnStart: false,
  groups: ["animation", "background"],
  schema: {
    duration: {
      type: "number",
      min: 0,
      default: 0,
    },
    delay: {
      type: "number",
      min: 0,
      default: 0,
    },
    start: {
      type: "number",
      min: 0,
      max: 1,
    },
    end: {
      type: "number",
      min: 0,
      max: 1,
    },
    from: {
      type: "number",
    },
    to: {
      type: "number",
    },
    ease: {
      type: "string",
    },
  },
};
/**
 * Pan balata on the X axis media elements on scroll
 */
function animate(kit, elements, duration, delay, _a = {}) {
  var { start, end, from, to, ease } = _a,
    params = __rest(_a, ["start", "end", "from", "to", "ease"]);
  const sequence = kit.sequence(params);
  // set sequence length to full duration
  sequence.add(kit.animate("BaseNone", elements, duration, delay), 0);
  // Place in animation on timeline
  if (end < start) {
    console.warn(
      `animations-kit: ${name}: end value ${end} should be larger than start value ${start}`
    );
  }
  const _duration = duration * Math.max(end - start, 0);
  const _delay = delay + duration * start;
  sequence.add(
    kit.animate("BasePosition", elements, _duration, _delay, {
      from: { x: from },
      to: { x: to },
      ease,
      force3D: true,
      immediateRender: true,
    }),
    0
  );
  return sequence.get();
}
export { name, properties, animate };
//# sourceMappingURL=baseBgPositionX.js.map
