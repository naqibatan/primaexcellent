...
import { keyframesToDuration, mapRange } from "../../../utils/animationsUtils";
const name = "Poke";
const properties = {
  schema: {},
};
...
function animate(kit, elements, duration, delay, _a = {}) {
  var { power, intensity = 0.5, direction = "right" } = _a,
    params = __rest(_a, ["power", "intensity", "direction"]);
  const sequence = kit.sequence(params);
  const { transformAxis, transformDirection } = directionMap[direction];
  const durations = keyframesToDuration(keyframes_translate, duration);
  const responsivePokeFactor = mapRange(
    0,
    1,
    powerMap.soft.pokeFactor,
    powerMap.hard.pokeFactor,
    intensity
  );
  const pokeFactor =
  ...
  keyframes_translate.forEach(({ translate }, index) => {
    sequence.add(
