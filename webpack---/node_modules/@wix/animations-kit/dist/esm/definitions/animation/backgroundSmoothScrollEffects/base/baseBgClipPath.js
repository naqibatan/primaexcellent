...
function animate(kit, elements, duration, delay, _a = {}) {
  var {
      ...
      clipParent,
      in: {
        start: in_start = undefined,
        end: in_end = undefined,
        ease: in_ease = undefined,
        direction: in_direction = undefined,
        minimum: in_minimum = properties.schema.in.properties.minimum.default,
      } = {},
      out: {
        start: out_start = undefined,
        end: out_end = undefined,
        ease: out_ease = undefined,
        direction: out_direction = undefined,
  ...
  if (!clipParent) {
    console.warn(
      `animations-kit: ${name}: "clipParent: element" is a mandatory parameter for this animation`
    );
  }
  const useClipRect = !!kit.frame.chrome; // is Chromium based browser
  const sequence = kit.sequence(params);
  const hasInAnimation = isNumber(in_start) && isNumber(in_end);
  const hasOutAnimation = isNumber(out_start) && isNumber(out_end);
  const compRect = kit.engine.getBoundingRect(clipParent);
  ...
  if (hasInAnimation) {
    if (in_end < in_start) {
      console.warn(
        `animations-kit: ${name}: "in" end value ${in_end} must be larger than "in" start value ${in_start}`
      );
    }
    ...
    sequence.add(
      kit.animate("BaseClipPath", elements, _duration, _delay, {
        from: inClip,
        to: baseClip,
        ease: in_ease,
        force3D: true,
  ...
  if (hasOutAnimation) {
    if (out_end < out_start) {
      console.warn(
        `animations-kit: ${name}: "out" end value ${out_end} must be larger than "out" start value ${out_start}`
      );
    }
