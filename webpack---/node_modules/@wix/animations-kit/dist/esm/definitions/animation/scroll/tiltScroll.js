...
function getYTravel(distance, power) {
  return (
    (typeof power !== "undefined" ? powerMap[power].travelY : distance) *
    MAX_Y_TRAVEL
  );
}
/**
...
function animate(kit, elements, _duration, _delay, _a = {}) {
  var { power, distance = 0, range = "in", direction = "right" } = _a,
    params = __rest(_a, ["power", "distance", "range", "direction"]);
  const sequence = kit.sequence(params);
  const { fromValues, toValues } = rangeValues[range];
  const [
    rotateXFrom,
    rotateYFrom,
    rotateZFrom,
    ...
    rotateYTo,
    rotateZTo,
  ] = [...fromValues.rotations, ...toValues.rotations];
  const [travelYFromDir, travelYToDir] = [fromValues.travelY, toValues.travelY];
  const dir = directionMap[direction];
  const rotateZFromStr = `${rotateZFrom < 0 ? "-=" : "+="}${
    Math.abs(rotateZFrom * ROTATION_Z) * dir
  }deg`;
  const rotateZToStr = `${rotateZTo < 0 ? "-=" : "+="}${
    Math.abs(rotateZTo * ROTATION_Z) * dir
  }deg`;
  ...
  sequence.add([
    ...
    kit.animate("BaseRotate", elements, 1000, 0, {
      from: { rotation: rotateZFromStr },
      to: { rotation: rotateZToStr },
      ease: "sineInOut",
    }),
    kit.animate("BasePosition", elements, 1000, 0, {
      from: { y: `${travelYFrom}vh` },
      to: { y: `${travelYTo}vh` },
      ease: "linear",
    }),
  ]);
...
const api = {
  getScrubOffsets({ power, range = "in", distance = 0 }) {
