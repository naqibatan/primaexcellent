...
function animate(kit, elements, duration, delay, _a = {}) {
  ...
  const unit = distance.type === "percentage" ? "%" : distance.type || "px";
  const elementsParams = elements.map((element) =>
    startFromOffScreen
      ? Object.assign(
          { element },
  ...
  sequence.add(
    kit.animate("BaseFade", elements, 0, 0, {
      from: { opacity: 0 },
      to: { opacity: 1 },
    })
  );
  sequence.add(
    elementsParams.map(({ element, x, y }) =>
