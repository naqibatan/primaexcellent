...
function animate(kit, elements, duration, delay, _a = {}) {
  ...
  const responsiveJelloFactor = mapRange(
    0,
    1,
    powerMap.soft.jelloFactor,
    powerMap.hard.jelloFactor,
    intensity
  );
  const jelloFactor =
  ...
  keyframes_skewY.forEach(({ skewY }, index) => {
    sequence.add(
      kit.animate("BaseSkew", elements, durations[index], 0, {
        to: { skewY: `${skewY * jelloFactor}deg` },
      })
    );
  });
