...
const rangeValues = {
  in: (rotation, travelY) => ({
    fromValues: {
      rotation3d: { rotationX: -2 * rotation, rotationY: -rotation },
  ...
  continuous: (rotation, travelY) => ({
    ...
    toValues: {
      ...
      rotation: { rotation: `+=${2 * rotation}deg` },
      travel: -travelY,
    },
  }),
};
...
function animate(kit, elements, _duration, _delay, _a = {}) {
  var { rotate = -100, power, range = "in", distance = 0 } = _a,
    params = __rest(_a, ["rotate", "power", "range", "distance"]);
  const sequence = kit.sequence(params);
  const { rotation, travelY } =
    typeof power !== "undefined"
      ? powerMap[power]
      : { rotation: rotate, travelY: distance };
  ...
  sequence.add([
    ...
    kit.animate("BasePosition", elements, 1000, 0, {
      from: { y: `${fromValues.travel}vh` },
      to: { y: `${toValues.travel}vh` },
      ease: "linear",
    }),
  ]);
  return sequence.get();
