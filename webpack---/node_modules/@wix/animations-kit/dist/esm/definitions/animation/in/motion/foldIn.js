...
function animate(kit, elements, duration, delay, _a = {}) {
  var { direction = "top", easing = "backOut", power, initialRotate = 90 } = _a,
    params = __rest(_a, ["direction", "easing", "power", "initialRotate"]);
  ...
  elements.forEach((element) => {
    ...
    const { x, y } = getElementTransformedPosition(
      paramsMap[adjDirection].origin,
      contentRect,
      deg2rad(elementAngleInDeg)
    );
    const transformOrigin = getTransformOriginTweenParams(
      compRect,
      contentRect,
      paramsMap[adjDirection].origin
    );
