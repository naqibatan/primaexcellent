...
import { transformPolarToXY } from "../../../utils/animationsUtils";
...
function animate(kit, elements, _duration, _delay, _a = {}) {
  var {
    ...
    params = __rest(_a, ["power", "distance", "angle", "range"]);
  ...
  const units = travel.type === "percentage" ? "%" : travel.type;
  sequence.add(
    kit.animate("BasePosition", elements, 1000, 0, {
      from: { x: `${fromValue.x}${units}`, y: `${fromValue.y}${units}` },
      to: { x: `${toValue.x}${units}`, y: `${toValue.y}${units}` },
      ease: "linear",
    })
  );
  return sequence.get();
