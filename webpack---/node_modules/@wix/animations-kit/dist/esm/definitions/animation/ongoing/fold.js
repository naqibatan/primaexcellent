...
function getDelaySequence(angle, time) {
  return [
    { fold: angle * -0.7, frameTime: time },
    { fold: angle * 0.6, frameTime: time },
    { fold: angle * -0.3, frameTime: time * 0.9 },
    { fold: angle * 0.2, frameTime: time * 0.8 },
    { fold: angle * -0.05, frameTime: time },
    { fold: 0, frameTime: time * 0.7 },
  ];
}
/**
...
function animate(kit, elements, duration, repeatDelay, _a = {}) {
  var {
    ...
    params = __rest(_a, ["direction", "power", "angle", "easing"]);
  const sequence = kit.sequence(params);
  const isResponsive = typeof power === "undefined";
  const { rotationAxis, transformOrigin } = directionMap[direction];
  ...
  elements.forEach((element) => {
    ...
    const { x, y } = getElementTransformedPosition(
      { x: origin[0], y: origin[1] },
      kit.engine.getBoundingRect(element),
      deg2rad(elementAngleInDeg)
    );
    sequence.add(
      kit.engine.set(element, {
        x,
        y,
        transformOrigin,
  ...
  sequence.add(
    kit.animate("BaseRotate3D", elements, duration / 4, 0, {
      to: { [rotationAxis]: `${rotation}deg` },
      ease: ease.out,
    })
  );
  if (repeatDelay) {
    ...
    delaySequence.forEach(({ fold, frameTime }) =>
      sequence.add(
        kit.animate("BaseRotate3D", elements, frameTime, 0, {
          to: { [rotationAxis]: `${fold}deg` },
          ease: "sineInOut",
        })
      )
    );
    ...
    sequence
      .add(
        kit.animate("BaseRotate3D", elements, duration / 2, 0, {
          to: { [rotationAxis]: `${-rotation}deg` },
          ease: ease.inOut,
        })
      )
