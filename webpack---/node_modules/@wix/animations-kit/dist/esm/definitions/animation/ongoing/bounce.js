...
function animate(kit, elements, duration, delay, _a = {}) {
  var { power, intensity = 0.3 } = _a,
    params = __rest(_a, ["power", "intensity"]);
  const sequence = kit.sequence(params);
  const durations = keyframesToDuration(keyframes_translateY, duration);
  const responsiveBounceFactor = mapRange(
    0,
    1,
    powerMap.soft.bounceFactor,
    powerMap.hard.bounceFactor,
    intensity
  );
  const bounceFactor =
  ...
  keyframes_translateY.forEach(({ translateY }, index) => {
    sequence.add(
      kit.animate("BasePosition", elements, durations[index], 0, {
        to: { y: (translateY * bounceFactor) / 2 },
        ease: "sineOut",
      })
    );
  });
