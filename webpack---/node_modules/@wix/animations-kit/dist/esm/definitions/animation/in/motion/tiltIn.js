...
function animate(kit, elements, duration, delay, _a = {}) {
  var { direction = "left", easing = "cubicOut" } = _a,
    params = __rest(_a, ["direction", "easing"]);
  const sequence = kit.sequence(params);
  sequence.add(
    kit.animate("BaseFade", elements, duration * 0.2, delay, {
      from: { opacity: 0 },
      to: { opacity: 1 },
      ease: "cubicOut",
    })
  ...
  elements.forEach((element) => {
    ...
    const position = getTransformTweenParams(
      contentRect,
      { dx: 0, dy: 1 },
      deg2rad(elementAngleInDeg),
      0.2
    );
    const { rotationZ } = paramsMap[adjDirection];
