...
function animate(kit, elements, duration, delay, _a = {}) {
  var { compRotation = 0, direction = "top-right", power = "medium" } = _a,
    params = __rest(_a, ["compRotation", "direction", "power"]);
  const partOneDuration = duration * 0.3;
  const partTwoDuration = duration - partOneDuration;
  const sequence = kit.sequence(params);
  sequence.add(
    kit.animate("BaseFade", elements, 0, 0, { to: { opacity: 0.01 } })
  );
  sequence.add(
    kit.animate("BaseFade", elements, partOneDuration, delay, {
      to: {
        opacity: 1,
      },
      ease: "cubicIn",
  ...
  elements.forEach((element) => {
    ...
    const sourcePoint = translatePoint(
      (bounds.width / 2) * normalizedDirection.x,
      (bounds.height / 2) * normalizedDirection.y,
      angleInDeg
    );
    const midPoint = translatePoint(
      (bounds.width / 3) * normalizedDirection.x,
      (bounds.height / 3) * normalizedDirection.y,
      angleInDeg
    );
    sequence.add(
      [
