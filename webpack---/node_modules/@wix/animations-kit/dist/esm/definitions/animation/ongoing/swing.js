...
function animate(kit, elements, duration, repeatDelay, _a = {}) {
  var { power, swing = 20, easing = "cubicInOut" } = _a,
    params = __rest(_a, ["power", "swing", "easing"]);
  const sequence = kit.sequence(params);
  const ease = getEasingFamily(easing);
  const swingDeg =
    typeof power !== "undefined" ? 20 * powerMap[power].swingFactor : swing;
  ...
  sequence
    .add(
      kit.animate("BaseRotate", elements, duration / 4, 0, {
        to: { rotation: `+=${swingDeg}deg` },
        ease: ease.out,
      })
    )
    .add(
      kit.animate("BaseRotate", elements, duration / 2, 0, {
    ...
    angles.forEach((angle, idx) => {
      ...
      sequence.add(
        kit.animate(
          "BaseRotate",
          elements,
          delaySequence[idx + 1].timeToFrame,
          0,
          {
            to: { rotation },
            ease: "sineInOut",
