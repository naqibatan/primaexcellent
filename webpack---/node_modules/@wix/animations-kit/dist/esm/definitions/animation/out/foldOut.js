...
function animate(kit, elements, duration, delay, _a = {}) {
  ...
  sequence.add(
    kit.animate(
      ...
      duration * 0.75,
      delay + duration * 0.25,
      { from: { opacity: 1 }, to: { autoAlpha: 0 }, ease: "Sine.easeInOut" }
    )
  ...
  elements.forEach((element) => {
    ...
    const transformXYParams = getElementTransformedPosition(
      paramsMap[adjDirection].origin,
      contentRect,
      elementAngleInRad
    );
    const originParams = getTransformOriginTweenParams(
      compRect,
      contentRect,
      paramsMap[adjDirection].origin
    );
