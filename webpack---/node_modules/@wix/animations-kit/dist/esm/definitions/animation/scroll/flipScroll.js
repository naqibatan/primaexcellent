...
const rangeValues = {
  in: (rotation) => ({ fromValue: -rotation, toValue: 0 }),
  out: (rotation) => ({ fromValue: 0, toValue: rotation }),
  continuous: (rotation) => ({
    fromValue: -rotation,
    toValue: rotation,
  }),
...
function animate(kit, elements, _duration, _delay, _a = {}) {
  var {
    ...
    params = __rest(_a, ["rotate", "direction", "power", "range"]);
  const sequence = kit.sequence(params);
  const rotationAxis = directionMap[direction];
  const { rotation } =
    typeof power !== "undefined" ? powerMap[power] : { rotation: rotate };
