...
import { getEasingFamily } from "../../../utils/animationsUtils";
const name = "Breathe";
const properties = {
  schema: {},
};
...
function getDistance({ value, type = "px" }, multiplier = 1) {
  const distance = value * multiplier;
  const units = type === "percentage" ? "%" : type;
...
function animate(kit, elements, duration, repeatDelay, _a = {}) {
  var {
    ...
    params = __rest(_a, ["direction", "distance", "easing"]);
  ...
  sequence
    .add(
      kit.animate("BasePosition", elements, duration / 4, 0, {
        to: { [transformAxis]: travel },
        ease: ease.out,
      })
    )
    .add(
      kit.animate("BasePosition", elements, duration / 2, 0, {
        to: { [transformAxis]: `-${travel}` },
        ease: ease.inOut,
      })
    );
  if (repeatDelay) {
    ...
    sequence
      .add(
        kit.animate("BasePosition", elements, duration / 2, 0, {
          to: { [transformAxis]: getDistance(distance, 0.5) },
          ease: ease.inOut,
        })
      )
      .add(
        kit.animate("BasePosition", elements, duration / 2, 0, {
          to: { [transformAxis]: getDistance(distance, -0.25) },
          ease: ease.inOut,
        })
      )
      .add(
        kit.animate("BasePosition", elements, duration / 3, 0, {
          to: { [transformAxis]: 0 },
          ease: ease.inOut,
        })
      )
