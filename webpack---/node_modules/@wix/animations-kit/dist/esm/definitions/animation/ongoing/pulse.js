...
function animate(kit, elements, duration, delay, _a = {}) {
  var { power, intensity = 0 } = _a,
    params = __rest(_a, ["power", "intensity"]);
  const sequence = kit.sequence(params);
  const durations = keyframesToDuration(keyframes_scale, duration);
  const responsivePulseOffset = mapRange(
    0,
    1,
    powerMap.soft.pulseOffset,
    powerMap.hard.pulseOffset,
    intensity
  );
  const pulseOffset =
  ...
  keyframes_scale.forEach(({ scale }, index) => {
    sequence.add(
      kit.animate("BaseScale", elements, durations[index], 0, {
        to: { scale: scale - (scale < 1 ? pulseOffset : 0) },
      })
    );
  });
