...
import { adapter as staticPageAdapterV2 } from "../../item-types/static-page-v2";
import { enrichWithSiteLevelData } from "../../adapters/utils";
import {
  ROBOTS_DIRECTIVES,
...
export var resolveIsIndexable = function (tags, adapterData) {
  ...
  var patternLevel = getIndexableValue(
    adapterData[adapter.IDs.ROBOTS_FROM_USER_PATTERN]
  ); // <String> 'noindex' / 'index' / 'max-snippet:20' (* defaults to undefined *)
  var overrides = getIndexableValue(
    getValueByIdentifier(tags, IDENTIFIERS.ROBOTS)
  ); // <String> 'noindex' / 'index' / 'max-snippet:20' (* defaults to undefined *)
  var isNoindex =
    siteLevel === NO_INDEX ||
    overrides === NO_INDEX ||
  ...
  if (isNoindex) {
    ...
    var robotsTagValueWithNoindex = getRobotsTagValueWithDirective(
...
export function getIndexableValue(value) {
  ...
  if (isRobotsDirectiveExists(value, ROBOTS_DIRECTIVES.NOINDEX)) {
    ...
    isRobotsDirectiveExists(value, INDEX) ||
