...
var INDEX = "index";
var noindexRelatedDirectiveNames = new Set([ROBOTS_DIRECTIVES.NOINDEX, INDEX]);
...
var sortDirectives = function (directives) {
  return __spreadArray([], directives, true).sort(function (
    firstDirective,
    otherDirective
  ) {
...
var getDirectiveNameValuePair = function (directive) {
  ...
  var nameValueSeparatorIndex = directive.lastIndexOf(
    ROBOTS_DIRECTIVE_NAME_VALUE_SEPARATOR
  );
...
var createDirective = function (directive, value) {
  return value !== undefined
    ? ""
        .concat(directive)
        .concat(ROBOTS_DIRECTIVE_NAME_VALUE_SEPARATOR)
        .concat(value)
    : directive;
};
...
var getDirectives = function (robotsTagValue) {
  return (
    robotsTagValue !== null && robotsTagValue !== void 0 ? robotsTagValue : ""
  )
    .split(" ")
...
var mergeDirectives = function (specificDirectives, generalDirectives) {
  ...
  var allDirectives = __spreadArray(
    __spreadArray([], generalDirectives, true),
    specificDirectives,
    true
  );
  allDirectives.forEach(function (directive) {
    if (isVariable(directive)) {
      ...
      Array.from(directiveNameToDirective.values()).forEach(function (
        currentDirective
      ) {
        if (isVariable(currentDirective)) {
          directiveNameToDirective.delete(
            getDirectiveNameFromDirective(currentDirective)
          );
        }
      });
    }
    ...
    directiveNameToDirective.set(
      getDirectiveNameFromDirective(directive),
      directive
    );
  });
  var mergedDirectives = Array.from(directiveNameToDirective.values());
  if (
    mergedDirectives.some(function (directive) {
...
export var mergeRobotsValues = function (
  specificRobotsValue,
  generalRobotsValue
) {
...
export var isRobotsDirectiveExists = function (robotsTagValue, directiveName) {
  return getDirectives(robotsTagValue).some(function (currentDirective) {
    var _a = getDirectiveNameValuePair(currentDirective),
      ...
      value = _a[1];
    return (
      name === directiveName && value !== REMOVE_ROBOTS_DIRECTIVE_OVERRIDE_VALUE
    );
  });
};
export var getDirectiveValue = function (robotsTagValue, directiveName) {
  var directive = getDirectives(robotsTagValue).find(function (
    currentDirective
  ) {
...
export var getRobotsTagValueWithDirective = function (
  robotsTagValue,
  directiveName,
  directiveValue
) {
...
export var getRobotsTagValueWithoutDirective = function (
  robotsTagValue,
  directiveName
) {
  var newDirectives = getDirectives(robotsTagValue).filter(function (
    directive
  ) {
...
export var formatRobotsTagForRendering = function (tags) {
  ...
  var directives = getDirectives(robotsTagValue)
    ...
    .filter(function (directive) {
      return (
        getDirectiveValueFromDirective(directive) !==
...
export var isRobotsTagValueSupported = function (robotsTagValue) {
  var supportedRobotDirectives = new Set(
    __spreadArray(
      __spreadArray([], Array.from(Object.values(ROBOTS_DIRECTIVES)), true),
      [INDEX],
      false
    )
  );
