...
import { clone } from "../renderer/utils/clone";
import { getValueByIdentifierFromContext } from "../tags/values/get-by-identifier";
import {
  SELF_REF_PATTERN_PREFIX,
  PATTERN_PREFIX,
  PATTERN_SUFFIX,
} from "../consts";
import { resolveSdSchema } from "./structured-data";
  ...
  blob.tags = blob.tags
    .map(function (_a) {
      ...
      var filledTag = __assign(
        ...
        isTagDisabled ? { disabled: isTagDisabled } : {}
      );
      return filledTag;
    })
    .filter(function (tag) {
...
export function fillPatternValue(value, context, tags, referencePath) {
  ...
  var patternRegex = new RegExp(
    "".concat(PATTERN_PREFIX, ".+?").concat(PATTERN_SUFFIX),
    "g"
  );
  ...
  function replacePatternWithValue(placeholder) {
    var isSelfRef = placeholder.startsWith("".concat(SELF_REF_PATTERN_PREFIX));
...
function isStructuredData(value) {
  ...
  return /"@context":"(http|https):\/\/(www.)?schema.org/.test(
    value.replace(/\s/g, "")
  );
}
function escapeForJson(value, contextValue) {
  ...
  return escapeHtml(contextValue, { strictForJson: true });
}
