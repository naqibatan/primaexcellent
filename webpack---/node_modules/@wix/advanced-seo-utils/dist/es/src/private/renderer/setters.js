...
import { isMultilingualTag } from "../tags/advanced/is-multilingual-tag";
import {
  getRobotsTagValueWithDirective,
  ROBOTS_DIRECTIVES,
} from "../tags/robots-tag/robots-tag";
import { INDEX } from "../consts/consts";
function updateValueByIdentifier(
  data,
  identifier,
  value,
  meta,
...
function setMetaTags(data, metaTags) {
  ...
  return __assign(__assign({}, data), { tags: tags });
}
...
function setExternalRouter(data, payload) {
  ...
  if (
    (title === null || title === void 0 ? void 0 : title.constructor.name) ===
    String.name
  ) {
    tags = setTitle({ tags: tags }, title).tags;
  }
  if (
    (metaTags === null || metaTags === void 0
      ? void 0
      : metaTags.constructor.name) === Array.name
  ) {
    tags = setMetaTags({ tags: tags }, metaTags).tags;
  }
  if (
    (links === null || links === void 0 ? void 0 : links.constructor.name) ===
    Array.name
  ) {
    tags = setLinks({ tags: tags }, links).tags;
  }
  if (
    (structuredData === null || structuredData === void 0
      ? void 0
      : structuredData.constructor.name) === Array.name
  ) {
    tags = setSchemas({ tags: tags }, structuredData).tags;
  }
  return __assign(__assign({}, data), { tags: tags });
}
