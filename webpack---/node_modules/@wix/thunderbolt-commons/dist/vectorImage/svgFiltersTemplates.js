...
function filterWrapperTemplate({ id, content, attrs = {} }) {
  ...
  return `<filter id="${id}" color-interpolation-filters="sRGB" ${attrsObjectToString(
    attrs
  )}>
...
function contrast({ value }) {
  return `<feComponentTransfer>${(0, svgFiltersParts_1.getContrast)(
    value
  )}</feComponentTransfer>`;
}
function brightness({ value, result }) {
  return `<feComponentTransfer ${attrToString("result", result)}>${(0,
  svgFiltersParts_1.getBrightness)(value)}</feComponentTransfer>`;
}
function saturation({ value, inAttr, result }) {
  return `<feColorMatrix type="saturate" values="${value}" ${
    inAttr ? `in="${inAttr}"` : ""
  }${result ? `result="${result}"` : ""}/>`;
}
function sepia({ value }) {
  return `<feColorMatrix type="matrix" values="${(0,
  svgFiltersParts_1.getSepia)(value)}"/>`;
}
function hue({ value }) {
...
function color({ value, inAttr, result }) {
  ...
  const opacity =
    typeof value === "object" && typeof value.opacity !== "undefined"
      ? value.opacity
      : 1;
  return `<feColorMatrix type="matrix" values="${(0,
  svgFiltersParts_1.getColor)(
    (0, svgFiltersParts_1.hex2RgbNorm)(hexColor),
    opacity
  )}" ${inAttr ? `in="${inAttr}"` : ""}${result ? `result="${result}"` : ""}/>`;
}
function tint({ value }) {
  return `<feColorMatrix type="matrix" values="${(0, svgFiltersParts_1.getTint)(
    (0, svgFiltersParts_1.hex2RgbNorm)(value)
  )}"/>`;
}
function blur({ value, inAttr }) {
  return `<feGaussianBlur stdDeviation="${value}" ${
    inAttr ? `in="${inAttr}"` : ""
  }/>`;
}
function alpha({ value, inAttr, result }) {
  return `<feComponentTransfer ${attrToString("in", inAttr)} ${attrToString(
    "result",
    result
  )}>${(0, svgFiltersParts_1.getAlpha)(value)}</feComponentTransfer>`;
}
function offset({ value, inAttr, result }) {
  return `<feOffset dx="${value.x}" dy="${value.y}" ${
    inAttr ? `in="${inAttr}"` : ""
  }${result ? `result="${result}"` : ""}/>`;
}
function blend({ value, inAttr, in2Attr, result }) {
  return `<feBlend mode="${value}" in="${inAttr}" in2="${in2Attr}" ${attrToString(
    "result",
    result
  )}/>`;
}
function composite({ value, inAttr, in2Attr, result }) {
  return `<feComposite operator="${value}" in="${inAttr}" in2="${in2Attr}" ${attrToString(
    "result",
    result
  )}/>`;
}
...
<feColorMatrix type="matrix" values="${(0, svgFiltersParts_1.getDoutone)(
    (0, svgFiltersParts_1.hex2RgbNorm)(light),
    (0, svgFiltersParts_1.hex2RgbNorm)(dark)
  )}" ${inAttr ? `in="${inAttr}"` : ""}${result ? `result="${result}"` : ""}/>`;
}
function luma({ value: { dark, light }, result }) {
  return `<feColorMatrix type="matrix" values="${(0,
  svgFiltersParts_1.getLumaMatrix)(light, dark)}" ${
    result ? `result="${result}"` : ""
  }/>`;
}
function shadow(_a) {
  var _b = _a.value,
    { blurRadius, mergeGraphic } = _b,
...
function getFilterValue(key, value, overrides) {
  if (key === "duotone") {
    return {
      light:
        ("duotoneLight" in overrides && overrides.duotoneLight) || value.light,
