...
function getSepia(amount) {
  return `${0.393 + 0.607 * (1 - amount)} ${0.769 - 0.769 * (1 - amount)} ${
...
function getContrast(amount) {
  return `<feFuncR type="linear" slope="${amount}" intercept="${
    Math.round((-0.5 * amount + 0.5) * 100) / 100
  }"/>
...
function getTint(color) {
  return `${1 - color.r} 0 0 0 ${color.r} ${1 - color.g} 0 0 0 ${color.g} ${
    1 - color.b
  } 0 0 0 ${color.b} 0 0 0 1 0`;
}
