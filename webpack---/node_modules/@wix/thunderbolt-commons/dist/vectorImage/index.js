...
exports.buildSvgUrlFactory =
  exports.getCSSStrokeAdjustments =
  exports.getCSSDropShadow =
  exports.encodeSVG =
  exports.getSvgContentForMask =
  exports.getScaledSvgViewBox =
  exports.getFilter =
  exports.removeWidthAndHeight =
  exports.parseSvgString =
  exports.getShapeDividerImageCss =
  exports.transformVectorImage =
  exports.getFontFamilyFontsFromSvg =
  exports.transformToTintColors =
  exports.trimSvgContents =
    void 0;
...
function transformToTintColors(svgString, resolvedColor) {
  ...
  return svgString.replace(fillAttributeMatcher, (__, colorToTint) => {
    ...
    if ((0, color_1.isGreyscale)(colorObj)) {
      ...
      const rTint = Math.floor(
        baseColor.red() + (255 - baseColor.red()) * tint
      ); // eslint-disable-line no-mixed-operators
      const gTint = Math.floor(
        baseColor.green() + (255 - baseColor.green()) * tint
      ); // eslint-disable-line no-mixed-operators
      const bTint = Math.floor(
        baseColor.blue() + (255 - baseColor.blue()) * tint
      ); // eslint-disable-line no-mixed-operators
...
function addA11yAttributes(svgString, altText) {
  ...
  if (typeof altText === "string") {
    newSvgString = newSvgString.replace(
      svgTagMatcher,
      `$1 aria-label="${escapeHtml(altText)}"$2`
    );
  }
  return newSvgString;
}
...
function getShadowFilter(filterId, shadow, layout, hexColor, mergeGraphic) {
  const filterString = (0, svgFilters_1.getShadow)(
    filterId,
    Object.assign(Object.assign({}, shadow), { color: hexColor, mergeGraphic })
  );
  return filterString.replace(
    /<filter /,
    `<filter ${getFilterRectInPixels(
      layout,
      shadow
...
function getScaledSvgViewBox(svgString, svgInfo, properties = {}) {
  ...
  if (svgString) {
    ...
    const preserveAspectRatio =
      aspectRatio || (displayMode === "stretch" ? "none" : "xMidYMid meet");
    let svgTag =
      (_a = svgString.match(svgTagMatcher)) === null || _a === void 0
        ? void 0
        : _a[0];
    if (svgTag) {
      if (preserveAspectRatioMatcher.test(svgTag)) {
        svgString = svgString.replace(
          svgTag,
          svgTag.replace(
            preserveAspectRatioMatcher,
            `$1${preserveAspectRatio}$3`
          )
        );
      } else {
        svgString = svgString.replace(
    ...
    if (svgType !== SVG_TYPES.UGC && !preserveViewBox && bbox) {
      ...
      svgString = svgString.replace(
        svgTag,
        svgTag.replace(viewBoxMatcher, `$1${bbox}$3`)
      );
    }
  }
  return svgString;
...
function wrapWithShadow({
  ...
  if (shadow && shadow.color) {
    ...
    const shadowFilter = getShadowFilter(
      filterId,
      shadow,
      layout,
      hexColor,
...
function getCSSDropShadow(shadow, colorsMap) {
  if (shadow && shadow.color && shadow.opacity) {
    ...
    return `drop-shadow(${shadow.x}px ${shadow.y}px ${
      shadow.blurRadius
    }px ${hexColor}${opacity.padStart(2, "0")})`;
  }
  return "";
}
...
function getCSSStrokeAdjustments(strokeWidth) {
  if (strokeWidth) {
    return {
      svgCalculatedPadding: `${Math.floor(strokeWidth / 2)}px ${Math.ceil(
        strokeWidth / 2
      )}px ${Math.ceil(strokeWidth / 2)}px ${Math.floor(strokeWidth / 2)}px`,
    ...
    if (styleMatch) {
      ...
      if (transformMatch) {
        replacement = topSvg[0].replace(
          styleMatch[0],
          styleMatch[0].replace(
            transformMatch[0],
            `transform: ${transform} ${transformMatch[1]}`
          )
        );
      } else {
        replacement = topSvg[0].replace(
...
function addSvgStyleNode(svgString, defsValue) {
  if (svgString && defsValue) {
    return svgString.replace(
      /(<svg[^>]*>)/,
      `$1<defs><style>${defsValue}</style></defs>`
    );
  }
  return svgString;
}
const DEFAULT_COLOR = "#242323";
const pickValidColors = (obj) =>
  Object.keys(obj).reduce((result, key) => {
    const colorValue = obj[key];
    if (
      (0, color_1.isColor)(colorValue) ||
      (0, color_1.isThemeColor)(colorValue)
    ) {
...
const transformVectorImage = (
  ...
  function withTintColors(svgString) {
    ...
    return svgInfo.svgType === SVG_TYPES.TINT
      ? transformToTintColors(svgString, color)
      : svgString;
  }
  function withCssOverrides(svgString) {
    const shouldOverrideColors =
      svgInfo.svgType === SVG_TYPES.COLOR &&
      Object.keys(overrideColors).length > 0;
    if (shouldOverrideColors) {
      const colors = Object.entries(overrideColors)
        .map(([colorName, rawColor]) => {
          const resolvedColor = (0, color_1.getFromColorMap)(
            rawColor,
            colorsMap
          );
          const index = colorName.replace("color", "");
          return `${(0, hoverBoxUtils_1.compIdToCssSelector)(
            compId,
            true
          )} svg [data-color="${index}"] {fill: ${resolvedColor};}`;
        })
        .join("\n");
...
function invertSvgContent(svgContent, x, y, width, height) {
  ...
  const pathTag =
    ((_a = svgContent.match(pathTagRe)) === null || _a === void 0
      ? void 0
      : _a[1]) || "";
  const pathData =
    ((_b = svgContent.match(pathDRe)) === null || _b === void 0
      ? void 0
      : _b[1]) || "";
  const rectPathSuffix = `M${x},${y - HAIRLINE_FIX_FACTOR} h${width} v${
    height + HAIRLINE_FIX_FACTOR
  } h${-width} Z`;
  return svgContent
...
function trimSvgContents(svgContent) {
  ...
  return svgContent
    .replace(svgContentStartRe, "<svg")
    .replace(svgContentEndRe, "</svg>");
}
...
const getShapeDividerImageCss = (
  ...
  const aspectRatio = keepAspectRatio
    ? shouldRepeat
      ? "xMinYMax meet"
      : "xMidYMax slice"
    : "none";
  ...
  const invertingTransform = shouldInvert
    ? `#${uniqueId} > g { transform: scaleY(-1) translateY(${-(
        2 * vbY +
        vbHeight
      )}px); } `
    : "";
  const pathColoring =
    svgInfo.svgType === "color" && !svgInfo.color2
      ? `, #${uniqueId} [data-color="1"] `
      : "";
  const svgStyle = `${invertingTransform}#${uniqueId}${pathColoring} { fill: ${color}; }`;
  const image = encodeSVG(
    removeWidthAndHeight(
      addSvgStyleNode(setSvgId(processedSvgContent, uniqueId), svgStyle),
      {
...
const buildSvgUrlFactory = () => {
  const addTrailingSlashIfMissing = (val) =>
    val.endsWith("/") ? val : `${val}/`;
  const getSvgBaseUrl = (mediaRootUrl) =>
    `${addTrailingSlashIfMissing(mediaRootUrl)}shapes/`;
