...
const isDisabledByEnabledDateRanges = ({ enabledDateRanges, date }) => {
...
export const getDisabledGridWithDisabledDays = (daysGrid, disabledDays) =>
  getDisabledGrid(daysGrid, (day) =>
    day && disabledDays.includes(day) ? "" : null
  );
export const getDisabledGridWithToday = (daysGrid, today) =>
  getDisabledGrid(daysGrid, (day) => (day && day !== today ? "" : null));
export const getDisabledGridWithMinMax = (daysGrid, minDay, maxDay) =>
  getDisabledGrid(daysGrid, (day) =>
    day && (day < minDay || day > maxDay) ? "" : null
  );
export const getDisabledGridWithDayOfWeek = (
  daysGrid,
  year,
  month,
  disabledDaysOfWeek
...
export const getDisabledGridWithPastDays = (daysGrid, day) =>
  getDisabledGrid(daysGrid, (weekDay) =>
    weekDay && weekDay < day ? "" : null
  );
export const getDisabledGridWithFutureDays = (daysGrid, day) =>
  getDisabledGrid(daysGrid, (weekDay) =>
    weekDay && weekDay > day ? "" : null
  );
const isDisabledByDisabledDateRanges = ({ disabledDateRanges, date }) => {
...
const isDisabledByDisabledDates = ({ disabledDates, date }) =>
  disabledDates.some((disabledDate) => areDatesEqual(disabledDate, date));
const isDisabledByDisabledDaysOfWeek = ({ disabledDaysOfWeek, dayOfWeek }) =>
...
export const isDateDisabled = (
  ...
  if (!assert.isNil(allowPastDates) || !assert.isNil(allowFutureDates)) {
    if (assert.isNil(todayDate)) {
      todayDate = timeZone
        ? new Date(
            new Date(Date.now()).toLocaleDateString("en-US", { timeZone })
          )
        : new Date(Date.now());
    }
...
export const convertDateRangesArray = (dateRanges, converter) =>
