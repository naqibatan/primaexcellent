    ...
    switch (date.type) {
      ...
      default:
        ...
        if (date.timeZone !== "Local") {
          nowDate = new Date(
            new TimeZone(date.timeZone).retrieveDateString(new Date(Date.now()))
          );
        } else {
...
export class TimeZone {
   ...
   */
  parseGMT(date) {
    const verboseFormat = (dateParam, timeZone) =>
      new Intl.DateTimeFormat("en-US", {
        timeZone,
        year: "2-digit",
        month: "2-digit",
    ...
    const dateInTimeZone = new Date(
      `${verboseFormat(date, this.timeZone)} UTC`
    );
    const gmtOffset =
      (dateInTimeZone.getTime() - dateInUTC.getTime()) / 3600000;
    const gmtOffsetString =
      Math.floor(gmtOffset) !== gmtOffset
  ...
  getDayAtTimeDate(date, time) {
    return new Date(
      `${this.retrieveDateString(date)} ${time} ${this.parseGMT(date)}`
    );
  }
  /**
   ...
   * @returns {String}
   */
  calculateGMT({ year, month, day, time }) {
    /**
