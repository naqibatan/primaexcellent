...
import { deletePropsSDKFactory } from "./deletePropsSDKFactory";
import { customClassListPropsSDKFactory } from "./customClassListPropsSDKFactory";
export const toJSONBase = ({ role, compType, isGlobal, isRendered }) => ({
  id: role,
  type: `$w.${compType}`,
  global: isGlobal(),
  rendered: isRendered(),
...
export const baseElementPropsSDKFactory = (api) => ({
  onMouseIn: (handler) =>
    registerCorvidMouseEvent("onMouseEnter", api, handler),
  onMouseOut: (handler) =>
    registerCorvidMouseEvent("onMouseLeave", api, handler),
