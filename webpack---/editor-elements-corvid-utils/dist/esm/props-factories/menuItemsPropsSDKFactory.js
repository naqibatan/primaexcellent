...
const _menuItemsPropsSDKFactory = (api) => {
  ...
  return {
    ...
    onItemMouseIn: (handler) =>
      registerCorvidMouseEvent("onItemMouseIn", api, handler, (payload) => ({
        item: transformPropDataToSdkData(payload),
        type: "itemMouseIn",
      })),
    onItemMouseOut: (handler) =>
      registerCorvidMouseEvent("onItemMouseOut", api, handler, (payload) => ({
        item: transformPropDataToSdkData(payload),
        type: "itemMouseOut",
      })),
    onItemClick: (handler) =>
      registerCorvidMouseEvent("onItemClick", api, handler, (payload) => ({
        item: transformPropDataToSdkData(payload),
        type: "itemMouseClick",
      })),
    onItemDblClick: (handler) =>
      registerCorvidMouseEvent("onItemDblClick", api, handler, (payload) => ({
        item: transformPropDataToSdkData(payload),
...
export function menuItemsPropsSDKFactory(api) {
  ...
  return withValidation(
    _menuItemsPropsSDKFactory,
    getMenuItemsSchema(menuItemsDepth),
    {
      menuItems: [
