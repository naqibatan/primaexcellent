...
export const checkIsTopBottomLink = (link) =>
  "anchorDataId" in link &&
...
export const extractPageLinkId = (link, metadata) => {
  if (checkLightboxLink(link)) {
    ...
    const { pagesMap, mainPageId } = metadata || {};
    if (!pagesMap) {
      return undefined;
    }
    const pageUrl = new URL(link.href ?? "");
    let value = Object.values(pagesMap).find(({ pageUriSEO }) => {
...
export const tryReportAnalyticsClicksBi = (reportBi, params, context) => {
  ...
  const biParamsDetails =
    details || linkDetails
      ? JSON.stringify({ ...linkDetails, ...details })
      : undefined;
  const biParams = {
    ...restParams,
  ...
  reportBi(
    { src: AnalyticsPromoteSrc, evid: ElementsClick, ...biParams },
    { endpoint: AnalyticsPromoteEndpoint, ...context }
  );
};
