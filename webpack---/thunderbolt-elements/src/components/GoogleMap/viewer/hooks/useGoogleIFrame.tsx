...
export function useGoogleIFrame(
  compRef: GoogleMapsRef,
  {
    mapData,
    isEditorMode,
  }: { mapData: GoogleMapProps['mapData']; isEditorMode?: boolean },
  {
    onUpdateZoom,
    onUpdateCenter,
    onMarkerClicked,
  ...
  const mergeWithCorvidLocations = (
    ...
    return fillLocationsWithIcon(corvidLocations || locations);
  };
  ...
  useImperativeHandle(compRef, () => ({
    ...
    setMarkers: (locations, options) => {
      ...
      sendMessage(
        JSON.stringify({
          ...modifiedMapData,
          locations: fillLocationsWithIcon(locations),
  ...
  useEffect(() => {
    ...
    sendMessage(
      JSON.stringify({
        ...modifiedMapData,
        locations: mergeWithCorvidLocations(modifiedMapData.locations),
        shouldKeepMarkers: shouldKeepLocations,
      }),
    );
  }, [modifiedMapData, sendMessage, iframeLoaded, prevMapData, isEditorMode]);
