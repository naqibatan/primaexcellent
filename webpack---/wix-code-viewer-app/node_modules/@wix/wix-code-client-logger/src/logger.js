...
const LogEvent = union("LogEvent", {
  BI: ({ biEvent }) => ({ biEvent }),
  Trace: ({ position, payload }) => ({ position, payload }),
...
const TracePosition = union("TracePosition", {
  ...
  End: ({ traceId, durationMs, result }) => ({ traceId, durationMs, result }),
});
...
const reportBI = (handlers) => (biEvent) => {
  const logEvent = LogEvent.BI({ biEvent });
...
const traceSync = (handlers) => (payload, fn) => {
  ...
  invokeHandlers(
    handlers,
    LogEvent.Trace({
      position: TracePosition.Start({ traceId }),
      payload,
    })
  );
...
const createLogger = ({ handlerCreators } = {}) => {
