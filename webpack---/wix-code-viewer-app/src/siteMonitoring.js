...
const { throttledLogSender } = require("./throttledLogSender");
const { create: createLogEntryFactory } = require("./logEntry");
...
function registerToConsoleMessages(callback, onWorkerLoggerLog) {
  return onWorkerLoggerLog(({ logLevel, args, stack }) => {
...
function registerToUncaughtRejections(callback) {
  const listener = (event) => {
    ...
    callback({ message, severity: siteMonitoringSeverity.ERROR });
  };
...
const registerListeners = ({ sendLog, onWorkerLoggerLog }) => {
...
const init = async ({
  ...
  try {
    ...
    const { createLogEntry } = createLogEntryFactory({
      wixSdk,
      metaSiteId,
      ignoredConsoleMessages,
    });
    ...
    const { sendLogThrottled } = throttledLogSender({
      appLogger,
    });

    const _sendLog = async ({ message, severity, sourceLocation }) => {
