...
const Environment = union("Environment", {
  NotInitialized() {},
  Initialized({ viewMode, biLoggerFactory }) {
    const biLogger = biLoggerFactory()
      .updateDefaults({ src: BI_SOURCE })
      .logger({
...
const biHandlerCreator = () => {
  ...
  const biHandler = () => ({
    init: ({ viewMode, biLoggerFactory }) => {
      // this "if" can be removed once we get rid of the legacy app
      if (biLoggerFactory) {
        environment = Environment.Initialized({ viewMode, biLoggerFactory });
      }
    },
    log: (logEvent) => {
      logEvent.matchWith({
        BI: ({ biEvent }) => {
          environment.matchWith({
            Initialized: ({ biLogger }) => {
              biLogger.log(biEvent, { useBatch: false });
            },
            NotInitialized: () => {
              throw new Error(
