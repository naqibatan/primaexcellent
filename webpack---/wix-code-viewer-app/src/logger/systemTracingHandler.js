...
const { union, Result } = require("@wix/wix-code-adt");
  ...
  const systemTracingHandler = () => ({
    ...
    log: filterByReportToHandlers(id, (logEvent) => {
      environment.matchWith({
        Initialized: ({ reportTrace }) => {
          logEvent.matchWith({
            Trace: ({ payload: { actionName }, position }) => {
              position.matchWith({
                Start: () =>
                  doTrace(reportTrace, { actionName, tracePosition: "before" }),
                End: ({ durationMs: actionDurationMs }) =>
                  doTrace(reportTrace, {
                    actionName,
                    tracePosition: "after",
                    actionDurationMs,
