...
const routerFactory = (
	...
	const isInternalValidRoute = (url: string): boolean => {
		const routeInfo = resolveUrl(url, routingConfig, { supportUrlHierarchy })
		...
		const validStaticUrl =
			relativeUrlDepth === 2 || (supportUrlHierarchy && routingConfig.routes[relativeUrlToCheck])
