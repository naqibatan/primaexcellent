...
import { getGlobalDefaults } from "./conduction-utils/global-defaults";
import { getGlobalRequestContext } from "./conduction-utils/global-defaults-checks";
import { getRequestContextQuery } from "./conduction-utils/request-context";
import { getCacheInstance } from "./experiments-cache";
import { tryParse } from "./experiments-data-utils";
import { httpRequest } from "./http-request";
import { getAllInScopePath, getExperimentPath } from "./urlMap";
import { ReadOnlyExperiments } from "./ReadOnlyExperiments";
var noop = function () {
  /* */
};
var Experiments = /** @class */ (function (_super) {
  ...
  Experiments.prototype._getUrlWithFallback = function (url, fallback) {
    ...
    var requestPromise = httpRequest(url).catch(function (error) {
      _this.onError(error);
      return fallback;
    });
  ...
  Experiments.prototype.load = function (scope) {
    ...
    var url = ""
      .concat(this.baseUrl)
      .concat(scopeConductionPath)
      .concat(requestContextQuery);
  ...
  Experiments.prototype.conduct = function (spec, fallbackValue) {
    ...
    var cachedResponse = getCacheInstance().getExperimentValue(
      spec,
      this.useNewApi
    );
    if (cachedResponse !== undefined) {
    ...
    var experimentsPath = getExperimentPath(
      spec,
      fallbackValue,
      this.useNewApi
    );
    ...
    var url = ""
      .concat(this.baseUrl)
      .concat(experimentsPath)
      .concat(requestContextQuery);
    var result = this._getUrlWithFallback(
      url,
      fallbackValue !== null && fallbackValue !== void 0 ? fallbackValue : ""
    ).then(function (response) {
