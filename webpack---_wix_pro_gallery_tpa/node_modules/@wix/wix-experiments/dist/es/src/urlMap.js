...
export var getAllInScopePath = function (scope, useNewApi) {
  ...
  var urlApi = useNewApi
    ? ""
        .concat(petriUrlMap.new.conductAllInScope, "?scope=")
        .concat(encodeURIComponent(scope))
    : ""
        .concat(petriUrlMap.old.conductAllInScope, "?scope=")
        .concat(encodeURIComponent(scope));
  return urlApi;
};
  ...
  return (
    url.indexOf(
      useNewApi
        ...
        : petriUrlMap.old.conductAllInScope
    ) !== -1
  );
}
export var getExperimentPath = function (key, fallback, useNewApi) {
  ...
  var urlApi = useNewApi
    ? ""
        .concat(petriUrlMap.new.conductExperiment, "?key=")
        .concat(encodeURIComponent(key), "&fallbackValue=")
        .concat(encodeURIComponent(fallback))
    : ""
        .concat(petriUrlMap.old.conductExperiment, "?key=")
        .concat(encodeURIComponent(key), "&fallback=")
        .concat(encodeURIComponent(fallback));
  return urlApi;
};
