...
var formatSrcToFilename = function formatSrcToFilename(src) {
  try {
    return isMediaSrc(src)
      ? src.split("/")[4].split("#").slice(0, -1).join("#")
      : "_.jpg"; // convert from wix:image://v1/[mediaUrl]/
  } catch (e) {
    return "_.jpg";
  }
};
var formatImageUrlToSrc = function formatImageUrlToSrc(mediaUrl, meta) {
  return (
    "wix:image://v1/" +
    (mediaUrl || utils.slugify(meta.name)) +
    "/" +
    utils.slugify(meta.fileName || meta.name) +
    "#originWidth=" +
    meta.width +
    "&originHeight=" +
    meta.height
...
var formatVideoUrlToSrc = function formatVideoUrlToSrc(videoUrl, meta) {
  return (
    "wix:video://v1/" +
    (videoUrl || utils.slugify(meta.name)) +
    "/" +
    utils.slugify(meta.fileName || meta.name) +
    "#posterUri=" +
    meta.posters[0].url +
...
var dimensionsFromUrl = function dimensionsFromUrl(url) {
  ...
  try {
    ...
    if (match && match[0] && match[0].length > 0) {
      var _match$0$replace$spli = match[0]
          .replace(/\//g, "")
          .split("_")
          .map(function (i) {
            return Number(i);
    ...
    try {
      ...
      url
        ...
        .forEach(function (dim) {
          dimensions[dim[0]] = isNaN(Number(dim[1]))
            ? String(dim[1])
            : Number(dim[1]);
        });
    } catch (e) {
...
var oldWixCodeItemToProGallery = function oldWixCodeItemToProGallery(
  wixCodeItem,
  i
) {
  return {
    itemId: utils.createUniqueUuidFromString(
      i + "_" + (wixCodeItem.uri || wixCodeItem.alt || i)
    ),
...
var newWixCodeItemToProGallery = function newWixCodeItemToProGallery(
  wixCodeItem,
  i
) {
  ...
  if (
    ((_wixCodeItem$type = wixCodeItem.type) == null
      ? void 0
      : _wixCodeItem$type.toLowerCase()) === "video" &&
    (((_wixCodeItem$src = wixCodeItem.src) != null &&
      _wixCodeItem$src.includes("youtube")) ||
      ((_wixCodeItem$src2 = wixCodeItem.src) != null &&
        _wixCodeItem$src2.includes("vimeo")))
  ) {
  ...
  var imageToken =
    wixCodeItem.imageToken || wixCodeItem.token || dimensions.token;
  var type = (wixCodeItem.type || "image").toLowerCase();
  var item = _objectSpread(
    {
      itemId:
        utils.slugify(wixCodeItem.slug) ||
        (utils.slugify(wixCodeItem.title || wixCodeItem.id) ||
          utils.createUniqueUuidFromString(
            wixCodeItem.src || wixCodeItem.html || i
          )) +
          "_" +
          i,
  ...
  if (type === "video") {
    ...
    var url = isValidImage(formatSrcToMediaUrl(wixCodeItem.thumbnail))
      ? formatSrcToMediaUrl(wixCodeItem.thumbnail)
      : dimensions.posterUri;
    ...
    if (item.isExternal || isMediaSrc(wixCodeItem.src)) {
      item.metaData.source =
        wixCodeItem.src.indexOf("youtube.com") > 0
          ? "youtube"
          : wixCodeItem.src.indexOf("vimeo.com") > 0
          ? "vimeo"
          : "wix";
      ...
      item.metaData.qualities = [
        {
          ...
          formats: [
            isMediaSrc(wixCodeItem.src)
              ? "wix"
              : wixCodeItem.src.split(/#|\?/)[0].split(".").pop().trim(),
          ],
        },
      ];
    }
...
export var wixCodeItemToProGallery = function wixCodeItemToProGallery(
  wixCodeItem,
  i
) {
  if (i === void 0) {
  ...
  return isNewWixCodeItemFormat(wixCodeItem)
    ? newWixCodeItemToProGallery(wixCodeItem, i)
    : oldWixCodeItemToProGallery(wixCodeItem, i);
};
export var wixCodeItemsToProGallery = function wixCodeItemsToProGallery(
  wixCodeItems
) {
  ...
  try {
    wixCodeItems.forEach(function (wixCodeItem, i) {
      ...
      if (wixCodeItem.secondaryMedia && item.metaData.type !== "video") {
        ...
        item.secondaryMedia = wixCodeItemToProGallery(
          wixCodeItem.secondaryMedia,
          i
        );
      }
...
export var proGalleryItemToWixCode = function proGalleryItemToWixCode(
  item,
  isNew
) {
  if (item === void 0) {
  ...
  return isNew
    ? proGalleryItemToNewWixCode(item)
    : proGalleryItemToOldWixCode(item);
};
export var proGalleryItemsToWixCode = function proGalleryItemsToWixCode(
  itemsDto,
  isNew
) {
  if (isNew === void 0) {
