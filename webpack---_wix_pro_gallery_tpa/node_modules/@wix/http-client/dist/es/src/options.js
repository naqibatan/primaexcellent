...
import { composeHeaders } from "./headers";
import {
  changeHost,
  extractHost,
  extractProtocol,
...
export const buildOptions = ({
  ...
  const newOptions = Object.assign(
    Object.assign(Object.assign({}, requestOptions), localConfigOptions),
    { headers }
  );
  return newOptions;
};
export const buildAllHeaders = (
  requestOptions,
  wixHeadersOpts,
  globalConfig = {
    httpMockServer: {
  ...
  return Object.assign(
    Object.assign(
      Object.assign({}, composedHeaders),
      lowerAllJsonKeys(headers)
    ),
    originalUrlHeader
  );
};
...
function buildUrlFromRequest(url, requestParams) {
  ...
  if (requestParams) {
    const params =
      toSearchQueryParams(requestParams) || requestParams.toString();
...
function lowerAllJsonKeys(objectToChange) {
  ...
  const result = Object.keys(objectToChange).reduce(
    (prev, key) =>
