...
function convertWixStyleParamsToMasterInfoElementsOptions(
  wixStyleParams,
  opts
) {
...
function designer_internal_sliderOrThumbnail_cancelMasterPaddingBecauseItIsHardCodedInCSS(
  master,
  galleryLayout
) {
...
function designer_external_slideshowAsureSocialFirstOnTop(
  master,
  galleryLayout
) {
...
function deleteFieldFromAllSections(master, fieldName) {
  Object.keys(master.order).forEach(function (section) {
    return (master.order[section] = master.order[section].filter(function (
      iType
    ) {
...
function designer_handleInternalVerticalCenterAndSocialTogether(
  master,
...
// }
function user_handleSocialParams(
  master,
  loveButton,
  allowSocial,
  loveCounter,
...
function designer_handleExternalPaddingWithImageInfoType(
  master,
  imageInfoType
) {
  ...
  if (
    imageInfoType === GALLERY_v4_CONSTS.infoType.SEPARATED_BACKGROUND ||
...
function getSection(iType, master) {
  ...
  var bottom = "bottom";
  return (
    [top, middle, bottom].find(function (sName) {
      return master.order[sName].includes(iType);
    }) || top
  ); // if nothing is found assuming top
}
...
function addNewInfoElementsConfig(bla, enabled, order) {
  ...
  masters.forEach(function (x) {
    ...
    if (supportLegacySitesFromTpa) {
      ...
      designer_handleInternalMasterPaddingForSocial(
        mas,
        getEnabled("Social", bla)
      );
      designer_handleInternalVerticalCenterAndSocialTogether(
        mas,
        bla.textsVerticalPadding
      );
      // slideshow
      ...
      // slider
      designer_internal_sliderOrThumbnail_cancelMasterPaddingBecauseItIsHardCodedInCSS(
        mas,
        bla.galleryLayout
      );
    }
...
function designer_disableTextsInOpositeInfo(master, externalPlacement) {
  if (
    (externalPlacement === "internal" && master.masterType === "external") ||
