...
import Consts, { galleryItemResolutionModesMap } from "./consts";
import {
  addPresetOptions,
  assignByString,
  mergeNestedObjects,
...
import { addCustomPresetStyles } from "./customPresetProcessor";
import coreOptions from "./coreOptions";
import { isHorizontalInfoCompatibleLayout } from "./proGalleryStyleBuilderUtils";
...
var convertToMobileSettingIfNeeded = function (isMobile, styles) {
  ...
  return { styles: styles, userSavedMobileStyles: userSavedMobileStyles };
};
export var getProGalleryStylesImp = function (
  wixStyles,
  gotStyleParams,
  options
) {
  ...
  function canSet(wixParam, stateParam) {
    ...
    if (isUndefined(stateParam)) {
      ...
      return (
        isUndefined(stateStyles[wixParam]) && !isUndefined(wixStyles[wixParam])
      );
    } else {
      ...
      return (
        isUndefined(stateStyles[stateParam]) &&
  ...
  if (canSet("allowOverlayGradient")) {
    ...
    if (allowOverlayGradient) {
      stateStyles.overlayBackground = "linear-gradient("
        .concat(overlayGradientDegrees, "deg,")
        .concat(overlayGradientColor1.value, ",")
        .concat(overlayGradientColor2.value, ")");
    } else {
  ...
  if (canSet("galleryMargin")) {
    stateStyles = assignByString(
      ...
      Number(wixStyles.galleryMargin)
    );
  }
  if (canSet("isRTL")) {
  ...
  if (canSet("rotatingGroupTypes")) {
    stateStyles = assignByString(
      stateStyles,
      "layoutParams_repeatingGroupTypes",
      String(wixStyles.rotatingGroupTypes)
    );
  }
  ...
  if (canSet("cubeRatio")) {
    stateStyles = assignByString(
      ...
      Number(eval(wixStyles.cubeRatio))
    );
  }
  ...
  if (canSet("numberOfDisplayedItems")) {
    stateStyles.numberOfDisplayedItems = Number(
      wixStyles.numberOfDisplayedItems
    );
  } else {
  ...
  if (stateStyles.imageRatioType === Consts.imageRatioType.RESPONSIVE) {
    stateStyles.gallerySliderImageRatio = "".concat(
      (100 / Number(stateStyles.numberOfDisplayedItems)).toFixed(2),
      "%/100%"
    );
  } else if (canSet("gallerySliderImageRatio", "cubeRatio")) {
    // stateStyles.imageRatioType === Consts.imageRatioType.FIXED
    stateStyles.gallerySliderImageRatio = Number(
      eval(
        ["16/9", "4/3", "1", "3/4", "9/16"][
    ...
    stateStyles.gallerySliderImageRatio = Number(
      eval(
        ["16/9", "4/3", "1", "3/4", "9/16"][
          Number(defaultStateStyles.gallerySliderImageRatio)
        ]
      )
    );
  }
  if (canSet("galleryImageRatio", "cubeRatio")) {
    stateStyles.galleryImageRatioFromWix = Number(
      eval(
        ["16/9", "4/3", "1", "3/4", "9/16"][Number(wixStyles.galleryImageRatio)]
      )
    );
  }
  ...
  if (String(wixStyles.galleryLayout) === "4") {
    stateStyles = assignByString(
      stateStyles,
      "layoutParams_cropRatio",
      stateStyles.gallerySliderImageRatio
    );
  } else if (
    String(wixStyles.galleryLayout) === "2" ||
    (isStoreGallery && wixStyles.galleryLayout === undefined)
  ) {
    stateStyles = assignByString(
      stateStyles,
      "layoutParams_cropRatio",
      stateStyles.galleryImageRatioFromWix ||
  ...
  if (canSet("galleryThumbnailsAlignment")) {
    stateStyles.galleryThumbnailsAlignment = ["bottom", "left", "top", "right"][
      ...
      if (canSet("fixedGalleryRatio")) {
        stateStyles = assignByString(
          ...
          1 /
            eval(
              ["16/9", "4/3", "1", "3/4", "9/16"][
                Number(wixStyles.fixedGalleryRatio)
              ]
            )
        );
      } else {
  ...
  var setTitlePlacementValue = function () {
    isHorizontalInfoCompatibleLayout(
      __assign(__assign({}, stateStyles), { isMobile: isMobile })
    )
  ...
  experimentsWrapper.getExperimentBoolean(
    "specs.pro-gallery.horizontalTitlePlacementOptionsViewer"
  )
  ...
  if (
    !isUndefined(stateStyles.allowTitle) &&
  ...
  if (canSet("arrowsContainerBackgroundColor")) {
    stateStyles = assignByString(
      ...
      wixStyles.arrowsContainerBackgroundColor.value
    );
  }
  if (canSet("arrowsContainerBorderRadius")) {
    stateStyles = assignByString(
      ...
      wixStyles.arrowsContainerBorderRadius
    );
  }
  ...
  if (canSet("scrollAnimation")) {
    ...
    var shouldNotAllowHorizontalScrollAnimation =
      String(stateStyles.galleryLayout) === "3" || // Thumbnails
      String(stateStyles.galleryLayout) === "5"; // Slideshow
  ...
  if (canSet("autoSlideshowType")) {
    ...
    var shouldAllowContinuousAutoSlide =
      stateStyles.slideAnimation === GALLERY_v4_CONSTS.slideAnimations.SCROLL;
  ...
  var fullResFeatureExperimentEnabled = experimentsWrapper.getExperimentBoolean(
    "specs.pro-gallery.enableFullResFeatureViewer"
  );
  var fullResFeatureEnviromentSupported = !isInEditor && !isSsr;
  var fullResFeatureStyleParamSet =
    wixStyles.galleryItemResolution !== undefined;
  var shouldUseFullResFeature =
    fullResFeatureExperimentEnabled &&
  ...
  if (shouldUseFullResFeature) {
    stateStyles = assignByString(
      ...
      galleryItemResolutionModesMap[wixStyles.galleryItemResolution]
    );
  }
  if (canSet("videoPlay")) {
  ...
  var getHeightByContent = function (stateStyles) {
    ...
    if (
      titlePlacement !== GALLERY_v4_CONSTS.placements.SHOW_ABOVE &&
...
export var getProGalleryStyles = function (wixStyles, options) {
  ...
  var spreadStyles = __assign(
    __assign(
      __assign(
        __assign(__assign({}, wixStyles.booleans), wixStyles.numbers),
        wixStyles.colors
      ),
      wixStyles.fonts
    ),
  ...
  var styles = getProGalleryStylesImp(
    spreadStyles,
    true,
