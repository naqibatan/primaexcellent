...
var tpaDimensionsHelper = /** @class */ (function () {
  ...
  tpaDimensionsHelper.prototype.getViewportScaleRatio = function (
    ...
    var shouldIgnoreRatio =
      renderUtils.shouldDebug("ignoreScaleRatio") ||
  ...
  tpaDimensionsHelper.prototype.protectGalleryWidth = function (width) {
    ...
    if (renderUtils.isMobile()) {
      maxGalleryWidth = Math.floor(
        maxGalleryWidth / this.getViewportScaleRatio()
      );
    }
    return Math.min(Math.floor(width), maxGalleryWidth);
  };
  tpaDimensionsHelper.prototype.protectGalleryHeight = function (
    height,
    offsetTop
  ) {
    if (offsetTop === void 0) {
  ...
  tpaDimensionsHelper.prototype.setWixHeight = function (_a) {
    ...
    var getMaxRowHeight = function () {
      ...
      var addLayoutHeight = function () {
        return (
          ...
          ["top", "bottom"].indexOf(styleParams.galleryThumbnailsAlignment) >= 0
    ...
    var getNeededHeight = function (requestedHeight, isResponsive) {
      var newHeight = Math.round(
        requestedHeight * _this.getViewportScaleRatio(isResponsive)
      );
      var should = false;
      ...
      if (
        (renderUtils.isEditor() || renderUtils.isInAlbumsBuilder()) &&
        maxRowHeight
      ) {
        if (styleParams.isInAdi) {
    ...
    var finalSetHeight = function (neededHeight, lastHeight) {
      if (neededHeight) {
        ...
        if (Math.abs(lastHeight - neededHeight) < 6 || neededHeight === 0) {
          if (renderUtils.isVerbose()) {
            console.log(
              "Skipping Wix height change: was too small " +
                lastHeight +
                ", now it's " +
                neededHeight
            );
    ...
    var clearHeightIfNeeded = function (
      isResponsive,
      isChangedLayout,
      ...
      if (isResponsive && isChangedLayout && allowHeightChangeOnNewLayout) {
        if (typeof clearHeight === "function") {
          ...
          console.warn(
            "trying to clearHeight() for responsive gallery, clearHeight is not a function"
          );
        }
      }
    };
    var shouldSetHeight = function () {
      ...
      var currentIsHorizontal =
        styleParams.scrollDirection ===
        GALLERY_v4_CONSTS.scrollDirection.HORIZONTAL;
      var scrollDirectionChanged =
        currentIsHorizontal !== _this.lastIsHorizontal;
      var infiniteScrollChanged =
        styleParams.enableInfiniteScroll !== _this.lastInfiniteScroll;
      ...
      var shouldSetOnLayoutChangeToHorizontal =
        !(isInfinite || isFixedHorizontlaGalleryRatio) &&
        !isResponsive &&
        isChangedLayout &&
        allowHeightChangeOnNewLayout;
      var should =
    ...
    if (shouldSetHeight.call(this)) {
      ...
      var finalHeight = getNeededHeight.call(
        this,
        height,
        !!styleParams.responsive
      );
