...
var getManualUpdatesData = function () {
  ...
  if (!manualUpdates) {
    manualUpdates = _.get(
      renderUtils.getGallerySettingsFromWindow(),
      "upgrades"
    );
  }
...
var ProGalleryVersionManager = /** @class */ (function () {
  function ProGalleryVersionManager(dateCreated, features) {
    ...
    this.versionManager = new VersionManagerCore(
      features,
      dateCreated,
      getManualUpdatesData()
    );
  }
  ProGalleryVersionManager.prototype.update = function (
    dateCreated,
    manualUpdates
  ) {
    ...
    this.versionManager.update(
      this.dateCreated,
      manualUpdates || getManualUpdatesData()
    );
  };
  ...
  ProGalleryVersionManager.prototype.getFeaturesWithNewerVersions = function (
    upgrades
  ) {
    var manualUpdates = getManualUpdatesData();
    var featuresWithNewerVersions =
      this.versionManager.getFeaturesWithNewerVersions(
        upgrades || manualUpdates
      );
...
var getPgVersionManager = function (dateCreatedTicks, currFeaturesList) {
  if (baseUtils && baseUtils.isDev()) {
    if (!window.proGalleryVersionManager) {
      var proGalleryVersionManager = new ProGalleryVersionManager(
        dateCreatedTicks,
        currFeaturesList
      );
