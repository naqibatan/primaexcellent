...
function getCreateEventFN($w, createEvent) {
  return (event) => {
    ...
    const { type } = event,
      data = __rest(event, ["type"]);
    return $w.createEvent(type, data);
  };
}
function withEventListeners(widgetApi, widgetMetaData, $w, createEvent) {
  var _a;
  (_a = widgetMetaData.events) === null || _a === void 0
    ? void 0
    : _a.forEach(({ name }) => {
        widgetApi[`on${pascalCase(name)}`] = (cb) =>
          $w.on(name, (event) => cb(createEvent(event)));
      });
  return widgetApi;
}
function mergeConsumerAndDefaultProps(widgetProps = {}, defaultProps) {
  ...
  return Object.entries(defaultProps).reduce(
    (mergedProps, [propKey, defaultValue]) => {
      const value = widgetProps[propKey];
      if (isPlainObject(value)) {
        mergedProps[propKey] = mergeConsumerAndDefaultProps(
          value,
          defaultValue
        );
      } else {
...
function shouldInvokePropsChangedOnUpdateConfig({ appParams }) {
  ...
  if (isBlocksAppParams(appParams)) {
    return (_b =
      (_a = appParams.appData) === null || _a === void 0
        ? void 0
        : _a.blocksConsumerData) === null || _b === void 0
      ? void 0
      : _b.invokePropsChangedOnUpdateConfig;
  }
  return false;
}
export async function createWidgetController(
  ...
  const normalizedDefaultProps = normalizeDefaultProps(
    widgetMetaData.properties
  );
  const mergedProps = mergeConsumerAndDefaultProps(
    config.props,
  ...
  try {
    const widgetController = await createWidgetFn(
      Object.assign({ $widget }, controllerConfig)
    );
    customPageReady =
      widgetController === null || widgetController === void 0
        ? void 0
        : widgetController.pageReady;
    moduleExports =
      widgetController === null || widgetController === void 0
        ? void 0
        : widgetController.exports;
    updateWidgetViewState =
      widgetController === null || widgetController === void 0
        ? void 0
        : widgetController.updateWidgetViewState;
  } catch (e) {
