...
var convertToPropertySchema = function (name, property) {
  ...
  var propertyDefinition = __assign(
    __assign({}, baseDefinitions[type]),
    restProps
  );
  var propertySchema = {
    structure:
      ((_a = {}),
      (_a[name] = createPropertyTemplate(propertyDefinition, isArray)),
      _a),
  };
  ...
  return function () {
    ...
    if (config.props) {
      Object.keys(config.props).forEach(function (propName) {
        ...
        if (propertyInfo.type) {
          apiStructure.propertiesSchemas.push(
            convertToPropertySchema(propName, propertyInfo)
          );
        }
      });
    }
    if (config.events) {
      Object.keys(config.events).forEach(function (event) {
        apiStructure.events.push(
          __assign({ name: event }, config.events[event])
        );
      });
    }
    if (config.methods) {
      Object.keys(config.methods).forEach(function (method) {
        apiStructure.functions.push(
          __assign(__assign({ name: method }, config.methods[method]), {
